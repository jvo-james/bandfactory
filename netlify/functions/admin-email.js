...
exports.handler=async event=>{
 ...
 try{
  ...
  if(action==='status'){
    ...
    const ref=core.db.collection('orders').doc(id),snap=await ref.get();if(!snap.exists)return json(404,{ok:false,error:'Order not found.'});const order={id:snap.id,...snap.data()};await ref.set({status,statusUpdatedAt:core.admin.firestore.FieldValue.serverTimestamp(),updatedAt:core.admin.firestore.FieldValue.serverTimestamp()},{merge:true});await core.db.collection('activity').add({action:'Order status updated',orderId:id,status,createdAt:core.admin.firestore.FieldValue.serverTimestamp()});let email={skipped:true};if(order.email){try{email=await core.sendStatus({...order,status},status)}catch(emailError){console.error('[Band Factory] Status saved but customer email failed',emailError);email={failed:true,error:String(emailError.message||emailError)}}}return json(200,{ok:true,email});
  }
  if(action==='broadcast'){
