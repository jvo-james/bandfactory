...
exports.handler=async event=>{
  ...
  try{
    ...
    const cloudName=process.env.CLOUDINARY_CLOUD_NAME,apiKey=process.env.CLOUDINARY_API_KEY,secret=process.env.CLOUDINARY_API_SECRET;if(!cloudName||!apiKey||!secret)throw new Error('Cloudinary environment variables are not configured.');
  ...
  }catch(error){console.error('[Band Factory] cloudinary-signature',error);return {statusCode:403,headers,body:JSON.stringify({ok:false,error:error.message||'Image upload could not be authorised.'})};}
};
