...
const publicOrder=o=>({
 id:o.id,status:o.status||'Preparing',payment:o.payment||'',name:o.name||'',createdAt:o.createdAt||'',fulfilment:o.fulfilment||'',fulfilmentPlan:o.fulfilmentPlan||'single',fulfilmentDate:o.fulfilmentDate||'',latestFulfilmentDate:o.latestFulfilmentDate||'',fulfilmentGroups:Array.isArray(o.fulfilmentGroups)?o.fulfilmentGroups.map(group=>({id:group.id||'',label:group.label||'Delivery',date:group.date||group.fulfilmentDate||'',status:group.status||o.status||'Preparing',items:(group.items||[]).map(i=>({name:i.name||'Band Factory item',color:i.color||'',size:i.size||'',style:i.style||'',qty:Number(i.qty||1),price:Number(i.price||0),image:i.image||''}))})):[],pickupDate:o.pickupDate||'',pickupAddress:o.pickupAddress||'',country:o.country||'',region:o.region||'',city:o.city||'',items:(o.items||[]).map(i=>({name:i.name||'Band Factory item',color:i.color||'',size:i.size||'',style:i.style||'',qty:Number(i.qty||1),price:Number(i.price||0),image:i.image||'',fulfilmentType:i.fulfilmentType||'',fulfilmentDate:i.fulfilmentDate||''})),subtotal:Number(o.subtotal||0),processingFee:Number(o.processingFee||0),deliveryFee:o.deliveryFee==null?null:Number(o.deliveryFee),deliveryFeeStatus:o.deliveryFeeStatus||'',total:Number(o.total||0)
});
