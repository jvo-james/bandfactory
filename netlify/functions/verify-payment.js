...
exports.handler=async event=>{
 ...
 try{
  ...
  const paymentRef=db.collection('paymentReferences').doc(reference),orderRef=db.collection('orders').doc(order.id),productRef=db.doc('products/smooth'),apparelRef=db.doc('products/spandexTubeTop'),reservationRef=db.collection('stockReservations').doc(order.id);
  await db.runTransaction(async tx=>{
   ...
   let stockResult=null,productSnap=null,apparelSnap=null;
   if(orderUsesManagedStock(order)&&!hasReservation){if(orderUsesSmoothStock(order))productSnap=await tx.get(productRef);if(orderUsesApparelStock(order))apparelSnap=await tx.get(apparelRef);if((!orderUsesSmoothStock(order)||productSnap?.exists)&&(!orderUsesApparelStock(order)||apparelSnap?.exists))stockResult=applyOrderToStock(order,productSnap?.data()||{},apparelSnap?.data()||{});}
   ...
   if(stockResult&&orderUsesSmoothStock(order))tx.set(productRef,{styles:stockResult.styles,colors:stockResult.styles.flat?.colors||{},updatedAt:t},{merge:true});if(stockResult&&orderUsesApparelStock(order))tx.set(apparelRef,{sizes:stockResult.sizes,updatedAt:t},{merge:true});
