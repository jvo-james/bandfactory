...
const {number,applyOrderToStock,orderUsesManagedStock,orderUsesSmoothStock,orderUsesRibbedStock,orderUsesCatalogStock,orderUsesApparelStock,validateWholesaleItems}=require('./order-stock');
...
exports.handler=async event=>{
 ...
 try{
  ...
  await db.runTransaction(async tx=>{
   ...
   if(needsWholesaleValidation||orderUsesRibbedStock(order)||orderUsesCatalogStock(order))catalogSnap=await tx.get(catalogRef);
   ...
   if(stockResult&&orderUsesSmoothStock(order))tx.set(productRef,{styles:stockResult.styles,colors:stockResult.styles.flat?.colors||{},updatedAt:t},{merge:true});if(stockResult&&(orderUsesRibbedStock(order)||orderUsesCatalogStock(order)))tx.set(catalogRef,{items:stockResult.catalog.items,updatedAt:t},{merge:true});if(stockResult&&orderUsesApparelStock(order))tx.set(apparelRef,{sizes:stockResult.sizes,updatedAt:t},{merge:true});
   ...
   if(orderUsesManagedStock(order)&&!hasReservation&&((orderUsesSmoothStock(order)&&!productSnap?.exists)||((orderUsesRibbedStock(order)||orderUsesCatalogStock(order))&&!catalogSnap?.exists)||(orderUsesApparelStock(order)&&!apparelSnap?.exists)))tx.set(db.collection('notifications').doc(),{type:'inventory',title:'Inventory setup needs attention',message:`${order.id} was paid, but a managed inventory document was not found. Please check stock manually.`,orderId:order.id,read:false,createdAt:t});
