...
exports.handler=async event=>{
 ...
 try{
  ...
  await db.runTransaction(async tx=>{
   ...
   let stockResult=null,productSnap=null,apparelSnap=null,catalogSnap=null;if(orderUsesManagedStock(latest)&&!hasReservation){if(orderUsesSmoothStock(latest))productSnap=await tx.get(productRef);if(orderUsesApparelStock(latest))apparelSnap=await tx.get(apparelRef);if(orderUsesCatalogStock(latest))catalogSnap=await tx.get(catalogRef);if((!orderUsesSmoothStock(latest)||productSnap?.exists)&&(!orderUsesApparelStock(latest)||apparelSnap?.exists))stockResult=applyOrderToStock(latest,productSnap?.data()||{},apparelSnap?.data()||{},catalogSnap?.exists?catalogSnap.data():defaultCatalog());}
   ...
   tx.set(paymentRef,{orderId,amount:paid,currency,createdAt:t},{merge:false});if(stockResult&&orderUsesSmoothStock(latest))tx.set(productRef,{styles:stockResult.styles,colors:stockResult.styles.flat?.colors||{},updatedAt:t},{merge:true});if(stockResult&&orderUsesApparelStock(latest))tx.set(apparelRef,{sizes:stockResult.sizes,updatedAt:t},{merge:true});if(stockResult&&orderUsesCatalogStock(latest))tx.set(catalogRef,{...stockResult.catalog,updatedAt:t},{merge:false});
