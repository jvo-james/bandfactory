...
exports.handler=async event=>{
 ...
 try{
  ...
  const orderRef=db.collection('orders').doc(orderId),paymentRef=db.collection('paymentReferences').doc(reference),productRef=db.doc('products/smooth'),catalogRef=db.doc('products/catalog'),apparelRef=db.doc('products/spandexTubeTop'),reservationRef=db.collection('stockReservations').doc(orderId);const snap=await orderRef.get();if(!snap.exists)return {statusCode:404,body:'Order not found'};
  ...
  await db.runTransaction(async tx=>{
   ...
   tx.set(paymentRef,{orderId,amount:paid,currency,createdAt:t},{merge:false});if(stockResult&&orderUsesSmoothStock(latest))tx.set(productRef,{styles:stockResult.styles,colors:stockResult.styles.flat?.colors||{},updatedAt:t},{merge:true});if(stockResult&&orderUsesRibbedStock(latest))tx.set(catalogRef,{items:stockResult.catalog.items,updatedAt:t},{merge:true});if(stockResult&&orderUsesApparelStock(latest))tx.set(apparelRef,{sizes:stockResult.sizes,updatedAt:t},{merge:true});
