...
const {number,applyOrderToStock,orderUsesManagedStock}=require('./order-stock');
...
exports.handler=async event=>{
 ...
 try{
  ...
  const orderRef=db.collection('orders').doc(orderId),paymentRef=db.collection('paymentReferences').doc(reference),productRef=db.doc('products/smooth');const snap=await orderRef.get();if(!snap.exists)return {statusCode:404,body:'Order not found'};
  ...
  await db.runTransaction(async tx=>{
   ...
   let stockResult=null,productSnap=null;if(orderUsesManagedStock(latest)){productSnap=await tx.get(productRef);if(productSnap.exists)stockResult=applyOrderToStock(latest,productSnap.data()||{});}
   const t=admin.firestore.FieldValue.serverTimestamp(),stockSyncStatus=orderUsesManagedStock(latest)?(stockResult?'updated':'needs-review'):'not-required';
   ...
   tx.set(paymentRef,{orderId,amount:paid,currency,createdAt:t},{merge:false});if(stockResult)tx.set(productRef,{styles:stockResult.styles,colors:stockResult.styles.flat?.colors||{},updatedAt:t},{merge:true});
   ...
   if(orderUsesManagedStock(latest)&&!productSnap?.exists)tx.set(db.collection('notifications').doc(),{type:'inventory',title:'Inventory setup needs attention',message:`${orderId} was paid, but products/smooth was not found. Please check stock manually.`,orderId,read:false,createdAt:t});
