...
exports.handler=async event=>{
 ...
 try{
  ...
  const orderRef=db.collection('orders').doc(orderId),paymentRef=db.collection('paymentReferences').doc(reference),productRef=db.doc('products/smooth'),reservationRef=db.collection('stockReservations').doc(orderId);const snap=await orderRef.get();if(!snap.exists)return {statusCode:404,body:'Order not found'};
  ...
  await db.runTransaction(async tx=>{
   ...
   let stockResult=null,productSnap=null;if(orderUsesManagedStock(latest)&&!hasReservation){productSnap=await tx.get(productRef);if(productSnap.exists)stockResult=applyOrderToStock(latest,productSnap.data()||{});}
   ...
   if(orderUsesManagedStock(latest)&&!hasReservation&&!productSnap?.exists)tx.set(db.collection('notifications').doc(),{type:'inventory',title:'Inventory setup needs attention',message:`${orderId} was paid, but products/smooth was not found. Please check stock manually.`,orderId,read:false,createdAt:t});
