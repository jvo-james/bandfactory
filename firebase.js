...
(function(){
  ...
  async function init(){
    ...
    window.__bfAuth=firebase.auth(); window.__bfDb=firebase.firestore();
    // Keep the admin signed in across browser restarts until they explicitly sign out.
    await window.__bfAuth.setPersistence(firebase.auth.Auth.Persistence.LOCAL);
    return true;
  }
  ...
  const api={ready,
    ...
    async list(collection,orderBy='createdAt',dir='desc'){if(await ready){let q=__bfDb.collection(collection);if(orderBy){try{q=q.orderBy(orderBy,dir)}catch{}}const s=await q.get();return s.docs.map(d=>({...d.data(),id:d.id}))}const arr=local.get(collection,[]);return orderBy?[...arr].sort((a,b)=>{const av=a?.[orderBy],bv=b?.[orderBy];return dir==='asc'?(av>bv?1:av<bv?-1:0):(bv>av?1:bv<av?-1:0)}):arr},
    async listAll(collection){if(await ready){const s=await __bfDb.collection(collection).get();return s.docs.map(d=>({...d.data(),id:d.id}))}return local.get(collection,[])},
    ...
    async reauthenticate(password){await ready;const user=__bfAuth?.currentUser;if(!user||!user.email)throw new Error('Please sign in again.');const credential=firebase.auth.EmailAuthProvider.credential(user.email,String(password||''));return user.reauthenticateWithCredential(credential)},
    async requestEmailChange(newEmail,currentPassword){await ready;const user=__bfAuth?.currentUser;if(!user)throw new Error('Please sign in again.');const next=String(newEmail||'').trim().toLowerCase();if(!next)throw new Error('Enter the new email address.');if(next===String(user.email||'').toLowerCase())throw new Error('That is already the current sign-in email.');await api.reauthenticate(currentPassword);await user.verifyBeforeUpdateEmail(next);return {pendingEmail:next,currentEmail:user.email}},
    async changePassword(currentPassword,newPassword){await ready;const user=__bfAuth?.currentUser;if(!user)throw new Error('Please sign in again.');const next=String(newPassword||'');if(next.length<6)throw new Error('Your new password must be at least 6 characters.');await api.reauthenticate(currentPassword);await user.updatePassword(next);return true},
    async currentAdmin(){await ready;if(!window.__bfAuth)return null;const user=__bfAuth.currentUser;if(!user)return null;const doc=await __bfDb.collection('admins').doc(user.uid).get();return doc.exists&&doc.data().active!==false?{...doc.data(),uid:user.uid,email:user.email}:null},
    onAuth(cb){ready.then(ok=>{if(!ok)return cb(null);__bfAuth.onAuthStateChanged(async u=>{if(!u)return cb(null);const d=await __bfDb.collection('admins').doc(u.uid).get();cb(d.exists&&d.data().active!==false?{...d.data(),uid:u.uid,email:u.email}:null)})})},
    async finalizeVerifiedOrder(order,verification={}){
      ...
      const catalogRef=__bfDb.doc('products/catalog');
