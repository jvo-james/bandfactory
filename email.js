...
const BFEmail=(()=>{
  ...
  return {
    ...
    async updateOrderStatus(orderId,status){
      ...
      // Keep the status write + customer notification in one authenticated
      // server-side request. This prevents the browser from saving the order
      // successfully and then failing before it gets a chance to send email.
      return request('/.netlify/functions/admin-email',{action:'status',orderId:id,status:next},true,{timeoutMs:15000});
    },
