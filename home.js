...
function homeVariantSwatches(product){
    const variants=BFCatalog.variants(product);if(variants.length<2)return '';
    const active=product.featuredVariantId||BFCatalog.featuredVariant(product)?.id||variants[0]?.id||'';
    return `<div class="bf-card-swatches home-product-swatches" aria-label="Choose colour">${variants.map(v=>`<button class="bf-card-swatch${v.id===active?' is-active':''}" type="button" data-card-variant="${escapeHTML(v.id)}" title="${escapeHTML(v.color)}" aria-label="${escapeHTML(v.color)}" aria-pressed="${v.id===active?'true':'false'}"><span style="--swatch:${escapeHTML(v.hex||'#ddd')}"></span><small>${escapeHTML(v.color)}</small></button>`).join('')}</div>`;
}
function homeMasterCards(products,cls='home-product-card',limit=6){
    const list=products.slice(0,limit);return list.map(product=>{const v=BFCatalog.featuredVariant(product),pre=BFCatalog.isPreorder(product),available=BFCatalog.purchasable(product,v?.id||''),price=BFCatalog.price(product,homeSettings),compare=BFCatalog.compareAtPrice(product,homeSettings),wh30=BFCatalog.wholesalePriceForQty(product,30),retailUrl=`item.html?id=${encodeURIComponent(product.id)}${v?.id?`&variant=${encodeURIComponent(v.id)}`:''}`,whUrl=`wholesale-product.html?id=${encodeURIComponent(product.id)}${v?.id?`&variant=${encodeURIComponent(v.id)}`:''}`,image=BFCatalog.variantImage(product,v?.id||'',homeCatalog,homeCategories),date=BFCatalog.preorderDate(product);const bothPrices=BFCatalog.isWholesale(product)&&wh30&&30>=BFCatalog.wholesale(product).minQty;return `<article class="${cls} bf-variant-card" data-bf-variant-card data-product-id="${escapeHTML(product.id)}"><a href="${retailUrl}" data-card-link><div class="image"><img data-card-image src="${homeEscape(image)}" alt="${homeEscape(product.name)}${v?.color?' in '+homeEscape(v.color):''}" loading="lazy">${pre?'<span class="home-stock-chip preorder-chip">Pre-order</span>':''}</div></a><div class="meta"><h3>${homeEscape(product.name)}</h3><p data-card-colour>${homeEscape(v?.color||product.subtitle||'')}</p>${homeVariantSwatches(product)}<div class="home-price-links">${price?`<a href="${retailUrl}" data-card-retail-link><span>Retail</span><strong>${BFCatalog.priceHtml(price,compare)}</strong></a>`:''}${bothPrices?`<a href="${whUrl}" data-card-wholesale-link><span>Wholesale · 30 pieces</span><strong>${homeEscape(BF.money(wh30))}</strong></a>`:''}</div>${pre&&date?`<small>Fulfilment from ${homeEscape(window.BFFulfilment?.shortDate?.(date)||date)}</small>`:''}<a class="add-mini stock-link" href="${retailUrl}">View product</a></div></article>`;}).join('');
}
