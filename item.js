...
async function initItem(){
  ...
  const categoryPage=currentItem.category==='ribbed'?'ribbed.html':currentItem.category==='tops'?'tops.html':'sets.html';const back=$id('itemBackLink');if(back){back.href=categoryPage;back.innerHTML=`<i class="fa-solid fa-arrow-left"></i> Back to ${currentItem.category==='ribbed'?'Ribbed':currentItem.category==='tops'?'Tops':'Sets'}`;}
  const relatedCatalog=['ribbed','tops','sets'].map(cat=>allItems.find(x=>x.category===cat&&x.id!==currentItem.id&&x.available!==false)).filter(Boolean);const smoothRec={name:'Black Smooth Flat Hairband',subtitle:'Smooth Hairband',image:BF.imageForProduct('flat','Black'),url:'product.html?color=Black&style=flat',price:Number(currentSettings.retailPrice||10)};const cards=[smoothRec,...relatedCatalog].slice(0,4);$id('itemRelated').innerHTML=cards.map(item=>{const isSmooth=!!item.url,url=isSmooth?item.url:(item.id==='spandex-tube-top'?'tube-top.html':`item.html?id=${encodeURIComponent(item.id)}`),image=isSmooth?item.image:BFCatalog.image(item),price=isSmooth?item.price:BFCatalog.price(item,currentSettings);return `<a class="catalog-tile" href="${url}"><div class="catalog-tile-media"><img src="${image}" alt="${item.name}" loading="lazy"></div><div class="catalog-tile-copy"><h3>${item.name}</h3><p>${item.subtitle||item.color||''}</p><strong>${price?BF.money(price):'View product'}</strong></div></a>`}).join('');
  updateBuy();
}
