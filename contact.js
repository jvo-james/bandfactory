async function initContact(){
  ...
  const email=st.storeEmail||cfg.contactEmail||'bandfactoryy@gmail.com';
...
document.addEventListener('DOMContentLoaded',()=>{initContact().catch(console.error);contactForm.onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,b=f.querySelector('button'),fd=new FormData(f),data={name:fd.get('name'),email:fd.get('email'),phone:fd.get('phone'),message:fd.get('message'),status:'new'};b.disabled=true;b.textContent='Sending...';try{const id=await BFStore.add('messages',data);await BFStore.notify('message','New contact message',`${data.name} sent a message.`,{email:data.email});await BFEmail.sendContactCustomer({id});contactSuccess.style.display='block';f.reset()}catch(err){console.error(err);BF.toast('Message could not be sent. Please try again.')}finally{b.disabled=false;b.textContent='Send message'}}})
