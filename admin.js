let DATA={orders:[],reviews:[],customers:[],subscribers:[],notifications:[],messages:[],activity:[],abandonedCarts:[],settings:{},colors:{},products:{colors:{},styles:{}},catalog:BFCatalog.mergeCatalog({})};
let loadingDepth=0;
...
async function loadAll(){
  return withAdminLoading(async()=>{
    ...
    DATA={orders,reviews,customers,subscribers:subs,notifications:notifs,messages:msgs,activity,abandonedCarts,settings,colors:colors.colors||{},products:colors||{colors:{},styles:{}},tubeTop,catalog:BFCatalog.mergeCatalog(catalogRaw||{}),sequence:sequence||{}};
...
function renderAll(){
  renderNavCounts();renderOverview();renderAnalytics();renderOrders();renderPendingPayments();renderTransactions();renderInternationalPayments();renderProducts();renderCatalogAdmin();renderTubeTopInventory();renderInventorySummary();renderWholesale();renderReviews();renderCustomers();renderAbandonedCarts();renderSubscribers();renderDelivery();renderMessages();renderNotifications();renderNotificationPopover();renderActivity();renderSettings();
}
...
function catalogField(value='',attrs=''){return `<input ${attrs} value="${String(value??'').replace(/&/g,'&amp;').replace(/"/g,'&quot;')}">`}
function renderCatalogAdmin(){
  const el=document.getElementById('catalogAdmin');if(!el)return;const c=DATA.catalog||BFCatalog.mergeCatalog({});
  const featured=(c.ribbed.featured||[]).map((p,i)=>`<div class="catalog-admin-card"><h4>${p.name}</h4><div class="form-grid"><div class="admin-field"><label>Name</label>${catalogField(p.name,`data-cat-featured-name="${i}"`)}</div><div class="admin-field"><label>Description</label><textarea data-cat-featured-desc="${i}">${p.description||''}</textarea></div><div class="admin-field"><label>Stock</label>${catalogField(p.stock,`type="number" min="0" data-cat-featured-stock="${i}"`)}</div><div class="admin-field"><label>Available</label><select data-cat-featured-available="${i}"><option value="true" ${p.available===false?'':'selected'}>Yes</option><option value="false" ${p.available===false?'selected':''}>No</option></select></div></div></div>`).join('');
  const ribbedColours=Object.entries(c.ribbed.colours||{}).map(([name,d])=>`<div class="catalog-stock-row"><strong>${name}</strong><input type="number" min="0" value="${Number(d.stock||0)}" data-ribbed-stock="${name}"><label><input type="checkbox" data-ribbed-available="${name}" ${d.available===false?'':'checked'}> Sell</label></div>`).join('');
  const basics=['tops','sets'].map(group=>`<div class="catalog-admin-group"><h3>Basics · ${group==='tops'?'Tops':'Sets'}</h3>${(c.basics[group]||[]).map(p=>{const sizes=Object.entries(p.sizes||{}).map(([size,d])=>`<div class="catalog-stock-row"><strong>${size}</strong><input type="number" min="0" value="${Number(d.stock||0)}" data-basic-stock="${p.id}" data-size="${size}"><label><input type="checkbox" data-basic-size-available="${p.id}" data-size="${size}" ${d.available===false?'':'checked'}> Sell</label><button type="button" class="small-btn" data-remove-size="${p.id}" data-group="${group}" data-size="${size}">Remove</button></div>`).join('')||'<p class="field-help">No size variants for this product.</p>';return `<div class="catalog-admin-card"><div class="panel-head"><div><h4>${p.name}</h4><small>${p.unitLabel||''}</small></div></div><div class="form-grid"><div class="admin-field"><label>Product name</label>${catalogField(p.name,`data-basic-name="${p.id}"`)}</div><div class="admin-field"><label>Price (GHS)</label>${catalogField(p.price,`type="number" min="0" data-basic-price="${p.id}"`)}</div><div class="admin-field catalog-wide"><label>Description</label><textarea data-basic-desc="${p.id}">${p.description||''}</textarea></div><div class="admin-field"><label>Colours (comma-separated)</label>${catalogField((p.colours||[]).join(', '),`data-basic-colours="${p.id}"`)}</div><div class="admin-field"><label>Available</label><select data-basic-available="${p.id}"><option value="true" ${p.available===false?'':'selected'}>Yes</option><option value="false" ${p.available===false?'selected':''}>No</option></select></div></div><div class="catalog-size-box"><strong>Sizes &amp; private stock</strong>${sizes}<div class="add-size-row"><input placeholder="New size e.g. 3XL" data-new-size="${p.id}"><button type="button" class="small-btn" data-add-size="${p.id}" data-group="${group}">Add size</button></div></div></div>`}).join('')}</div>`).join('');
  el.innerHTML=`<div class="catalog-admin-group"><h3>Ribbed Hairbands</h3><div class="form-grid"><div class="admin-field"><label>Retail price (GHS)</label>${catalogField(c.ribbed.price,`type="number" min="0" id="catalogRibbedPrice"`)}</div><div class="admin-field"><label>Description</label><textarea id="catalogRibbedDescription">${c.ribbed.description||''}</textarea></div><div class="admin-field"><label>Available</label><select id="catalogRibbedAvailable"><option value="true" ${c.ribbed.available===false?'':'selected'}>Yes</option><option value="false" ${c.ribbed.available===false?'selected':''}>No</option></select></div></div><h4>Signature prints</h4>${featured}<h4>Solid colours</h4><div class="catalog-stock-grid">${ribbedColours}</div></div>${basics}`;
  el.querySelectorAll('[data-add-size]').forEach(b=>b.onclick=()=>{const input=el.querySelector(`[data-new-size="${b.dataset.addSize}"]`),size=String(input.value||'').trim().toUpperCase();if(!size)return BF.toast('Enter a size first.');const p=BFCatalog.findBasic(DATA.catalog,b.dataset.addSize);p.sizes ||= {};p.sizes[size]={stock:0,available:true};renderCatalogAdmin()});
  el.querySelectorAll('[data-remove-size]').forEach(b=>b.onclick=()=>{const p=BFCatalog.findBasic(DATA.catalog,b.dataset.removeSize);delete p.sizes[b.dataset.size];renderCatalogAdmin()});
}
async function saveCatalog(){return withAdminLoading(async()=>{const c=JSON.parse(JSON.stringify(DATA.catalog));c.ribbed.price=Number(document.getElementById('catalogRibbedPrice').value||0);c.ribbed.description=document.getElementById('catalogRibbedDescription').value;c.ribbed.available=document.getElementById('catalogRibbedAvailable').value==='true';(c.ribbed.featured||[]).forEach((p,i)=>{p.name=document.querySelector(`[data-cat-featured-name="${i}"]`).value;p.description=document.querySelector(`[data-cat-featured-desc="${i}"]`).value;p.stock=Number(document.querySelector(`[data-cat-featured-stock="${i}"]`).value||0);p.available=document.querySelector(`[data-cat-featured-available="${i}"]`).value==='true'});Object.entries(c.ribbed.colours||{}).forEach(([name,d])=>{d.stock=Number(document.querySelector(`[data-ribbed-stock="${name}"]`).value||0);d.available=document.querySelector(`[data-ribbed-available="${name}"]`).checked});for(const group of ['tops','sets'])for(const p of c.basics[group]||[]){p.name=document.querySelector(`[data-basic-name="${p.id}"]`).value;p.price=Number(document.querySelector(`[data-basic-price="${p.id}"]`).value||0);p.description=document.querySelector(`[data-basic-desc="${p.id}"]`).value;p.colours=document.querySelector(`[data-basic-colours="${p.id}"]`).value.split(',').map(x=>x.trim()).filter(Boolean);p.available=document.querySelector(`[data-basic-available="${p.id}"]`).value==='true';for(const [size,d] of Object.entries(p.sizes||{})){d.stock=Number(document.querySelector(`[data-basic-stock="${p.id}"][data-size="${size}"]`).value||0);d.available=document.querySelector(`[data-basic-size-available="${p.id}"][data-size="${size}"]`).checked}}await BFStore.setDoc('products/catalog',c,false);await BFStore.log('Catalog products updated');BF.toast('Catalog products saved');await loadAll()},'Saving catalog products…')}
