...
function renderTubeTopInventory(){
  ...
  el.innerHTML=order.map(size=>{const d=sizes[size]||{};return `<div class="tube-size-admin-row simple"><div><strong>${size}</strong><small>${Number(d.stock??0)>0?'Available':'Sold out'}</small></div><label>Pieces <input type="number" min="0" step="1" data-tube-stock="${size}" value="${Math.max(0,Number(d.stock??0))}"></label><label class="availability-check"><input type="checkbox" data-tube-available="${size}" ${d.available===false?'':'checked'}><span>Sell this size</span></label></div>`}).join('');
