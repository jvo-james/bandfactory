...
async function loadAll(){
  return withAdminLoading(async()=>{
    const [orders,reviews,customers,subs,notifs,msgs,activity,abandonedCarts,settings,colors,sequence,tubeTopRaw,catalogRaw]=await Promise.all([
      ...
      BFStore.getDoc('settings/store',{}),BFStore.getDoc('products/smooth',{colors:{}}),BFStore.getDoc('settings/orderSequence',{}),BFStore.getDoc('products/spandexTubeTop',null),BFStore.getDoc('products/catalog',{})
    ]);
...
function renderAll(){
  renderNavCounts();renderOverview();renderAnalytics();renderOrders();renderPendingPayments();renderTransactions();renderInternationalPayments();renderProducts();renderCatalogProducts();renderTubeTopInventory();renderInventorySummary();renderWholesale();renderReviews();renderCustomers();renderAbandonedCarts();renderSubscribers();renderDelivery();renderMessages();renderNotifications();renderNotificationPopover();renderActivity();renderSettings();
}
...
function renderTubeTopInventory(){
  const el=document.getElementById('tubeTopAdminStock');if(!el)return;const priceInput=document.getElementById('tubeTopPriceAdmin'),descInput=document.getElementById('tubeTopDescriptionAdmin');if(priceInput)priceInput.value=Number(DATA.tubeTop?.price??64);if(descInput)descInput.value=DATA.tubeTop?.description||'Double lined and stretchy';const order=['XS','S','M','L','XL','2XL'],sizes=DATA.tubeTop?.sizes||{};
  el.innerHTML=order.map(size=>{const d=sizes[size]||{};return `<div class="tube-size-admin-row"><div><strong>${size}</strong><small>${Number(d.stock??0)>0?'Available':'Sold out'}</small></div><label>Pieces <input type="number" min="0" step="1" data-tube-stock="${size}" value="${Math.max(0,Number(d.stock??0))}"></label><label>Width cm <input data-tube-width="${size}" value="${escapeAdminValue(DATA.tubeTop?.sizeGuide?.[size]?.width||'')}"></label><label>Length cm <input data-tube-length="${size}" value="${escapeAdminValue(DATA.tubeTop?.sizeGuide?.[size]?.length||'')}"></label><label class="availability-check"><input type="checkbox" data-tube-available="${size}" ${d.available===false?'':'checked'}><span>Sell this size</span></label></div>`}).join('');
...
async function saveTubeTopInventory(){return withAdminLoading(async()=>{const sizes={};document.querySelectorAll('[data-tube-stock]').forEach(input=>{const size=input.dataset.tubeStock,available=document.querySelector(`[data-tube-available="${size}"]`);sizes[size]={stock:Math.max(0,Number(input.value||0)),available:!!available?.checked}});const sizeGuide={};Object.keys(sizes).forEach(size=>sizeGuide[size]={width:document.querySelector(`[data-tube-width="${size}"]`)?.value.trim()||'',length:document.querySelector(`[data-tube-length="${size}"]`)?.value.trim()||''});const product={...DATA.tubeTop,name:'Spandex Tube Top',price:Number(document.getElementById('tubeTopPriceAdmin')?.value||64),color:'Black',description:document.getElementById('tubeTopDescriptionAdmin')?.value||'Double lined and stretchy',sizes,sizeGuide,updatedAt:new Date().toISOString()};await BFStore.setDoc('products/spandexTubeTop',product,false);DATA.tubeTop=product;renderTubeTopInventory();renderInventorySummary();BF.toast('Spandex Tube Top stock saved.');},'Saving tube top stock…')}
...
function renderCatalogProducts(){
  ...
  el.innerHTML=categories.map(category=>`<div class="catalog-category-group" data-catalog-group="${category}">${(DATA.catalog||[]).filter(x=>x.category===category&&x.id!=='spandex-tube-top').map(item=>`<article class="admin-catalog-card" data-catalog-id="${item.id}"><div class="admin-catalog-card-head"><img src="${BFCatalog.image(item)}" alt="${escapeAdminValue(item.name)}"><div><h4>${escapeAdminValue(item.name)}</h4><small>${category==='ribbed'?'Ribbed Hairband':category==='tops'?'Top':'Set'}</small></div></div><div class="admin-catalog-grid"><div class="admin-field"><label>Product name</label><input data-field="name" value="${escapeAdminValue(item.name)}"></div><div class="admin-field"><label>Price (GHS)</label><input data-field="price" type="number" min="0" value="${item.price??(item.category==='ribbed'?DATA.settings.ribbedPrice||'':'')}"></div><div class="admin-field"><label>Colour / pack colours</label><input data-field="color" value="${escapeAdminValue(item.color||'')}"></div><div class="admin-field"><label>Available?</label><select data-field="available"><option value="true" ${item.available!==false?'selected':''}>Yes</option><option value="false" ${item.available===false?'selected':''}>No</option></select></div><div class="admin-field" style="grid-column:1/-1"><label>Description</label><textarea data-field="description">${escapeAdminValue(item.description||'')}</textarea></div><input data-field="category" type="hidden" value="${category}"><div class="admin-field"><label>Image key</label><input data-field="imageKey" value="${escapeAdminValue(item.imageKey||item.id)}"><p class="field-help">Change the image path in images.js.</p></div></div>${item.sizes?`<div class="admin-size-list"><strong>Sizes and stock</strong><div data-size-rows>${Object.entries(item.sizes).map(([size,d])=>`<div class="admin-size-row"><input data-size-name value="${escapeAdminValue(size)}" aria-label="Size"><input data-size-stock type="number" min="0" value="${Number(d.stock||0)}" aria-label="Stock"><input data-size-width value="${escapeAdminValue(item.sizeGuide?.[size]?.width||'')}" placeholder="Width cm" aria-label="Width in centimetres"><input data-size-length value="${escapeAdminValue(item.sizeGuide?.[size]?.length||'')}" placeholder="Length cm" aria-label="Length in centimetres"><label class="availability-check"><input data-size-available type="checkbox" ${d.available===false?'':'checked'}><span>Sell</span></label></div>`).join('')}</div><button class="small-btn" type="button" onclick="addCatalogSizeRow('${item.id}')">+ Add size</button><div class="admin-field"><label>Size guide introduction</label><textarea data-field="sizeDescription">${escapeAdminValue(item.sizeDescription||'')}</textarea></div></div>`:`<div class="admin-field"><label>Stock</label><input data-field="stock" type="number" min="0" value="${Number(item.stock??0)}"></div>`}</article>`).join('')||'<p>No products in this category yet.</p>'}</div>`).join('');
...
function applyInventoryProductTab(tab){
  ...
  const tube=document.querySelector('.tube-top-admin-panel'),settings=document.querySelector('.product-settings-panel'),catalog=document.getElementById('catalogAdminList')?.closest('.admin-panel'),heading=document.querySelector('.colour-section-heading'),colors=document.getElementById('colorAdminGrid');
  if(tube)tube.hidden=tab!=='tops';if(settings)settings.hidden=tab!=='smooth';if(catalog)catalog.hidden=tab==='smooth';if(heading)heading.hidden=tab!=='smooth';if(colors)colors.hidden=tab!=='smooth';const ch=document.getElementById('catalogAdminHeading');if(ch)ch.textContent=tab==='ribbed'?'Ribbed Hairbands':tab==='tops'?'Tops':tab==='sets'?'Sets':'Products';const add=document.getElementById('addCatalogProduct');if(add)add.hidden=tab==='smooth';
  document.querySelectorAll('[data-catalog-group]').forEach(g=>g.hidden=g.dataset.catalogGroup!==tab);
}
function addCatalogSizeRow(id){const card=document.querySelector(`[data-catalog-id="${id}"]`),rows=card?.querySelector('[data-size-rows]');if(!rows)return;rows.insertAdjacentHTML('beforeend','<div class="admin-size-row"><input data-size-name placeholder="New size"><input data-size-stock type="number" min="0" value="0"><input data-size-width placeholder="Width cm"><input data-size-length placeholder="Length cm"><label class="availability-check"><input data-size-available type="checkbox" checked><span>Sell</span></label></div>')}
window.addCatalogSizeRow=addCatalogSizeRow;
async function saveCatalogProducts(){return withAdminLoading(async()=>{const existing=Object.fromEntries((DATA.catalog||[]).map(x=>[x.id,x]));document.querySelectorAll('[data-catalog-id]').forEach(card=>{const id=card.dataset.catalogId,item={...(existing[id]||{}),id};card.querySelectorAll('[data-field]').forEach(input=>{const k=input.dataset.field;if(k==='price')item[k]=input.value===''?null:Number(input.value);else if(k==='stock')item[k]=Math.max(0,Number(input.value||0));else if(k==='available')item[k]=input.value==='true';else item[k]=input.value});const rows=[...card.querySelectorAll('.admin-size-row')];if(rows.length){item.sizes={};item.sizeGuide={};rows.forEach(r=>{const size=r.querySelector('[data-size-name]')?.value.trim();if(size){item.sizes[size]={stock:Math.max(0,Number(r.querySelector('[data-size-stock]')?.value||0)),available:!!r.querySelector('[data-size-available]')?.checked};item.sizeGuide[size]={width:r.querySelector('[data-size-width]')?.value.trim()||'',length:r.querySelector('[data-size-length]')?.value.trim()||''}}})}existing[id]=item});const items=Object.values(existing);await BFStore.setDoc('products/catalog',{items,updatedAt:new Date().toISOString()},false);await BFStore.log('Catalog products updated');BF.toast('Ribbed, tops and sets saved.');await loadAll()},'Saving catalog products…')}
...
function addCatalogProduct(){
  const category=window.activeInventoryProductTab;if(!['ribbed','tops','sets'].includes(category))return;
  const name=prompt(`Name of the new ${category==='ribbed'?'ribbed hairband':category==='tops'?'top':'set'}:`);if(!name?.trim())return;
  const id=(name.trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||`product-${Date.now()}`)+'-'+Date.now().toString().slice(-4);
  const item={id,category,name:name.trim(),subtitle:'',price:null,color:'',description:'',available:true,imageKey:category==='ribbed'?'ribbed-'+id:category==='tops'?'placeholder-top':'placeholder-set',featuredOrder:99};
  if(category!=='ribbed')item.sizes={S:{stock:0,available:true}};
  DATA.catalog=[...(DATA.catalog||[]),item];renderCatalogProducts();applyInventoryProductTab(category);BF.toast('New product added. Add its details, then save.');
}
document.addEventListener('DOMContentLoaded',()=>document.getElementById('addCatalogProduct')?.addEventListener('click',addCatalogProduct));
