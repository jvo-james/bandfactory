...
function renderOrderFulfilment(o){
  ...
  return `<section class="admin-order-fulfilment"><div class="admin-order-fulfilment-head"><div><span class="drawer-section-label">Fulfilment plan</span><h3>${groups.length>1?'Multiple deliveries':label}</h3></div><span class="admin-fulfilment-plan-badge">${groups.length>1?'Separate deliveries':(plan==='together'&&summary.hasPreorder?'Everything together':'Single fulfilment')}</span></div><div class="admin-fulfilment-groups">${groups.map((group,index)=>{const groupItems=(group.items||[]).map(item=>`<div class="admin-fulfilment-item"><span>${item.name}${item.size?` · ${item.size}`:''}</span><strong>×${Number(item.qty||1)}</strong></div>`).join('');return `<div class="admin-fulfilment-group"><div class="admin-fulfilment-group-head"><strong>${group.label||`${label} ${index+1}`}</strong><span>${fmtDate(group.date||o.fulfilmentDate||o.pickupDate)}</span></div>${groupItems||'<p>No item details saved.</p>'}<small>Status follows the overall order status.</small></div>`}).join('')}</div>${o.fulfilment==='delivery'&&o.deliveryFeeStatus?`<p class="admin-fulfilment-note"><i class="fa-solid fa-circle-info"></i><span>Delivery fee: ${o.deliveryFeeStatus}.</span></p>`:''}</section>`;
}
...
async function saveOrderStatus(id){
  return withAdminLoading(async()=>{const v=document.getElementById('drawerStatus').value;const result=await BFEmail.updateOrderStatus(id,v);applyOrderStatusLocally(id,v);const local=DATA.orders.find(o=>o.id===id);if(local?.fulfilmentGroups?.length)local.fulfilmentGroups=local.fulfilmentGroups.map(group=>({...group,status:v}));renderAll();closeOrder();BF.toast(result?.email?.failed?'Order status saved · email needs retry':result?.email?.skipped?'Order status saved':'Order status saved · customer emailed')},'Saving order…');
}
...
function openProductStudio(id=''){
  const existing=(DATA.catalog||[]).find(p=>p.id===id)||{},isNew=!existing.id,categories=studioLiveCategories().filter(c=>c.id!=='smooth'),wholesale=BFCatalog.wholesale(existing),fulfilment=BFCatalog.fulfilment(existing);
  openStudioModal(`<form id="studioProductForm" class="studio-form"><div class="studio-form-head"><span>Product editor</span><h2>${isNew?'Add a new product':'Edit product'}</h2><p>Work through the essentials below. Extra details stay neatly grouped so this panel remains easy to use.</p></div><div class="studio-cover-edit product"><img id="studioProductPreview" src="${escapeAdminValue(BFCatalog.image(existing)||'images/placeholder-product.svg')}" alt=""><div class="studio-upload-zone"><strong>Main product image</strong><p>Choose the main photo customers should see first.</p><label class="small-btn pink">Choose image<input hidden type="file" accept="image/*" onchange="uploadStudioAsset(this,'studioProductPreview','studioProductImage')"></label></div></div><input id="studioProductImage" type="hidden" value="${escapeAdminValue(existing.image||'')}"><input id="studioProductImages" type="hidden" value="${escapeAdminValue(JSON.stringify(existing.images||[]))}"><div class="studio-gallery-editor"><div><strong>More product images</strong><p>Add extra angles if you have them.</p></div><label class="small-btn">+ Add image<input hidden type="file" accept="image/*" onchange="uploadStudioAsset(this,'','',false,true)"></label><div id="studioGalleryList" class="studio-gallery-list"></div></div><div class="studio-form-grid"><div class="admin-field"><label>Product name</label><input id="studioProductName" required value="${escapeAdminValue(existing.name||'')}" placeholder="Product name"></div><div class="admin-field"><label>Category</label><select id="studioProductCategory">${categories.map(c=>`<option value="${escapeAdminValue(c.id)}" ${existing.category===c.id?'selected':''}>${escapeAdminValue(c.name)}</option>`).join('')}</select></div><div class="admin-field sale-current-field"><label>Current selling price (GHS)</label><input id="studioProductPrice" type="number" min="0" step="0.01" value="${existing.price??''}" placeholder="0.00"><p class="field-help">What the customer pays now.</p></div><div class="admin-field sale-old-field"><label>Old price before discount (GHS) <span class="optional-label">Optional</span></label><input id="studioProductCompareAtPrice" type="number" min="0" step="0.01" value="${existing.compareAtPrice??''}" placeholder="Leave blank when not on sale"><p class="field-help">Enter only a higher former price. It will appear crossed out beside the current price.</p></div><div class="admin-field"><label>Customer availability</label><select id="studioProductAvailable"><option value="true" ${existing.available!==false?'selected':''}>Available to buy</option><option value="false" ${existing.available===false?'selected':''}>Not available</option></select></div><div class="admin-field"><label>Short detail / colour</label><input id="studioProductSubtitle" value="${escapeAdminValue(existing.subtitle||existing.color||'')}" placeholder="e.g. Black"></div><div class="admin-field"><label>Display order</label><input id="studioProductOrder" type="number" min="1" value="${Number(existing.featuredOrder||99)}"></div><div class="admin-field span-2"><label>Description</label><textarea id="studioProductDescription" placeholder="Tell customers about this product">${escapeAdminValue(existing.description||'')}</textarea></div></div><details class="studio-detail-group" open><summary><span><strong>Sizes &amp; quantities</strong><small>Add only the sizes this product comes in.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body"><div class="studio-size-quick"><span>Quick add</span>${['XS','S','M','L','XL','2XL'].map(size=>`<button type="button" onclick="addStudioSizeRow('${size}')">${size}</button>`).join('')}<button class="custom" type="button" onclick="addStudioSizeRow('')"><i class="fa-solid fa-plus"></i> Custom size</button></div><div id="studioSizeRows" class="studio-size-rows">${studioSizeRowsHtml(existing.sizes||{})}</div><div id="studioSizeEmpty" class="studio-size-empty" ${Object.keys(existing.sizes||{}).length?'hidden':''}><i class="fa-solid fa-ruler-combined"></i><strong>No sizes added</strong><span>If this item has sizes, use a quick size above or add a custom one.</span></div><button class="small-btn studio-add-size" type="button" onclick="addStudioSizeRow('')"><i class="fa-solid fa-plus"></i> Add another size</button></div></details><details class="studio-detail-group"><summary><span><strong>Stock &amp; display extras</strong><small>Use these only when needed.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body studio-form-grid"><div class="admin-field studio-simple-stock"><label>One-size quantity</label><input id="studioProductStock" type="number" min="0" value="${Number(existing.stock||0)}"><p class="field-help">For products that do not use sizes.</p></div><div class="admin-field"><label>Pack size</label><input id="studioProductPack" type="number" min="1" value="${Number(existing.packSize||1)}"></div><div class="studio-ribbed-fields span-2"><strong>Ribbed style quantities</strong><p>Only used for Ribbed products with separate Flat and Twisted stock.</p><div class="studio-ribbed-grid"><div class="admin-field"><label>Flat quantity</label><input id="studioRibbedFlatStock" type="number" min="0" value="${Number(existing.styles?.flat?.stock??existing.stock??0)}"></div><div class="admin-field"><label>Flat availability</label><select id="studioRibbedFlatAvailable"><option value="true" ${existing.styles?.flat?.available===false?'':'selected'}>Available</option><option value="false" ${existing.styles?.flat?.available===false?'selected':''}>Unavailable</option></select></div><div class="admin-field"><label>Twisted quantity</label><input id="studioRibbedTwistedStock" type="number" min="0" value="${Number(existing.styles?.twisted?.stock??0)}"></div><div class="admin-field"><label>Twisted availability</label><select id="studioRibbedTwistedAvailable"><option value="true" ${existing.styles?.twisted?.available===false?'':'selected'}>Available</option><option value="false" ${existing.styles?.twisted?.available===false?'selected':''}>Unavailable</option></select></div></div></div></div></details><details class="studio-detail-group" open><summary><span><strong>Wholesale</strong><small>Choose whether this product can be bought in bulk and set the customer-facing wholesale prices.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body"><div class="studio-form-grid"><div class="admin-field"><label>Available for wholesale</label><select id="studioWholesaleEnabled"><option value="false" ${wholesale.enabled?'':'selected'}>No</option><option value="true" ${wholesale.enabled?'selected':''}>Yes</option></select><p class="field-help">When enabled, this product appears on Other Products Wholesale. For Ribbed hairbands, use the dedicated Hairbands Wholesale setup.</p></div></div><div id="studioWholesaleSettings" ${wholesale.enabled?'':'hidden'}><div class="studio-wholesale-tier-head"><div><strong>Wholesale price tiers</strong><span>Use the first tier as the minimum order quantity. A higher quantity uses the latest tier it qualifies for.</span></div><button type="button" class="small-btn" onclick="addStudioWholesaleTier()"><i class="fa-solid fa-plus"></i> Add price tier</button></div><div id="studioWholesaleTiers" class="studio-wholesale-tier-rows">${studioWholesaleTierRowsHtml(wholesale)}</div></div></div></details><details class="studio-detail-group"><summary><span><strong>Fulfilment</strong><small>Control whether customers pay for this now as a normal item or as a pre-order.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body studio-form-grid"><div class="admin-field"><label>Fulfilment type</label><select id="studioFulfilmentType"><option value="standard" ${fulfilment.type==='standard'?'selected':''}>Standard / ready stock</option><option value="preorder" ${fulfilment.type==='preorder'?'selected':''}>Pre-order</option></select><p class="field-help">Pre-order items can be paid for before their stock arrives.</p></div><div class="admin-field" id="studioPreorderDateWrap" ${fulfilment.type==='preorder'?'':'hidden'}><label>Pre-order fulfilment date</label><input id="studioPreorderDate" type="date" value="${escapeAdminValue(fulfilment.date||'')}"><p class="field-help">Customers will see this date on the product, bag, checkout, receipt and tracking.</p></div></div></details><div class="studio-form-actions">${!isNew?'<button class="small-btn danger" type="button" id="deleteStudioProduct">Remove product</button>':''}<span></span><button class="small-btn" type="button" onclick="closeStudioModal()">Cancel</button><button class="small-btn primary" type="submit"><i class="fa-solid fa-check"></i> Save product</button></div></form>`);
...
async function saveStudioProduct(e,id){
  ...
  return withAdminLoading(async()=>{let wholesale,fulfilment;try{wholesale=category==='ribbed'?{enabled:false,minQty:1,tiers:[]}:collectStudioWholesale();fulfilment={type:document.getElementById('studioFulfilmentType')?.value==='preorder'?'preorder':'standard',date:document.getElementById('studioFulfilmentType')?.value==='preorder'?(document.getElementById('studioPreorderDate')?.value||''):''};if(fulfilment.type==='preorder'&&!fulfilment.date)throw new Error('Choose a fulfilment date for this pre-order.');}catch(error){return BF.toast(error.message||'Please check the wholesale and fulfilment settings.')}let cleanId=id||`${studioSlug(category)}-${studioSlug(name)}`;if(!id&&DATA.catalog.some(p=>p.id===cleanId))cleanId+=`-${Date.now().toString().slice(-4)}`;const existing=DATA.catalog.find(p=>p.id===id)||{},sizes=collectStudioSizes(),stock=Math.max(0,Number(document.getElementById('studioProductStock')?.value||0)),available=document.getElementById('studioProductAvailable').value==='true';const item={...existing,id:cleanId,category,name,subtitle:document.getElementById('studioProductSubtitle').value.trim(),color:document.getElementById('studioProductSubtitle').value.trim(),description:document.getElementById('studioProductDescription').value.trim(),price:document.getElementById('studioProductPrice').value===''?null:Number(document.getElementById('studioProductPrice').value),compareAtPrice:document.getElementById('studioProductCompareAtPrice')?.value===''?null:Number(document.getElementById('studioProductCompareAtPrice')?.value||0),available,stock,packSize:Math.max(1,Number(document.getElementById('studioProductPack').value||1)),featuredOrder:Math.max(1,Number(document.getElementById('studioProductOrder').value||99)),image:document.getElementById('studioProductImage').value||existing.image||'',images:studioGalleryImages(),wholesale,fulfilment};if(Object.keys(sizes).length)item.sizes=sizes;else delete item.sizes;if(category==='ribbed'){const flatStock=Math.max(0,Number(document.getElementById('studioRibbedFlatStock')?.value||0)),twistedStock=Math.max(0,Number(document.getElementById('studioRibbedTwistedStock')?.value||0)),flatAvailable=document.getElementById('studioRibbedFlatAvailable')?.value==='true',twistedAvailable=document.getElementById('studioRibbedTwistedAvailable')?.value==='true';item.styles=item.styles||{};item.styles.flat={...(item.styles.flat||{}),stock:flatStock,available:flatAvailable};item.styles.twisted={...(item.styles.twisted||{}),stock:twistedStock,available:twistedAvailable};item.stock=flatStock;item.available=flatAvailable;}else if(existing.category==='ribbed'){delete item.styles;}if(id)DATA.catalog=DATA.catalog.map(p=>p.id===id?item:p);else DATA.catalog.push(item);await persistStudioProducts();await BFStore.log(id?'Catalog product updated':'Catalog product created',{productId:cleanId});closeStudioModal();BF.toast(`${name} saved.`);renderCatalogProducts();renderCatalogStudio()},'Saving product…');
}
