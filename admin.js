...
function renderOrderItem(item,index){
  ...
  if(item.type==='wholesale-product'){
    const preorder=BFFulfilment?.isPreorder?.(item),date=BFFulfilment?.itemFulfilmentDate?.(item,'');
    return `<article class="order-product-card wholesale-product-card"><div class="order-product-head"><span class="order-product-number">${index+1}</span><div><strong>${item.name||'Wholesale product'}</strong><small>Other products wholesale${item.size?` · Size ${item.size}`:''}</small></div><b>${BF.money(itemTotal)}</b></div><div class="wholesale-summary-grid"><span><small>Units</small><strong>${qty}</strong></span><span><small>Unit price</small><strong>${BF.money(item.price||0)}</strong></span><span><small>Order type</small><strong>Wholesale</strong></span><span><small>Fulfilment</small><strong>${preorder?'Pre-order':'Ready stock'}</strong></span></div>${preorder?`<div class="standard-mix-note"><i class="fa-regular fa-calendar-days"></i><span>Pre-order fulfilment: ${fmtDate(date)}</span></div>`:''}</article>`;
  }
...
function renderOrderFulfilment(o){
  const summary=window.BFFulfilment?.summary?.(o)||{groups:[]};
  const groups=summary.groups||[];
  const plan=o.fulfilmentPlan==='separate'?'separate':'together';
  if(!groups.length)return '';
  const label=o.fulfilment==='pickup'?'Pickup':'Delivery';
  return `<section class="admin-order-fulfilment"><div class="admin-order-fulfilment-head"><div><span class="drawer-section-label">Fulfilment plan</span><h3>${groups.length>1?'Multiple deliveries':label}</h3></div><span class="admin-fulfilment-plan-badge">${groups.length>1?'Separate deliveries':(plan==='together'&&summary.hasPreorder?'Everything together':'Single fulfilment')}</span></div><div class="admin-fulfilment-groups">${groups.map((group,index)=>{const groupItems=(group.items||[]).map(item=>`<div class="admin-fulfilment-item"><span>${item.name}${item.color?` · ${item.color}`:''}${item.size?` · ${item.size}`:''}</span><strong>×${Number(item.qty||1)}</strong></div>`).join('');return `<div class="admin-fulfilment-group"><div class="admin-fulfilment-group-head"><strong>${group.label||`${label} ${index+1}`}</strong><span>${fmtDate(group.date||o.fulfilmentDate||o.pickupDate)}</span></div>${groupItems||'<p>No item details saved.</p>'}<small>Order status controls the overall order. Future delivery dates stay scheduled until their planned date.</small></div>`}).join('')}</div>${o.fulfilment==='delivery'&&o.deliveryFeeStatus?`<p class="admin-fulfilment-note"><i class="fa-solid fa-circle-info"></i><span>Delivery fee: ${o.deliveryFeeStatus}.</span></p>`:''}</section>`;
}
...
function renderTubeTopInventory(){
  ...
  const priceInput=document.getElementById('tubeTopPriceAdmin'),compareInput=document.getElementById('tubeTopCompareAtPriceAdmin'),descInput=document.getElementById('tubeTopDescriptionAdmin');
  if(priceInput)priceInput.value=Number(DATA.tubeTop?.price??64);
  if(compareInput)compareInput.value=Number(DATA.tubeTop?.compareAtPrice||0)||'';
...
async function saveTubeTopInventory(){return withAdminLoading(async()=>{const entries=[...document.querySelectorAll('[data-tube-stock]')].map(input=>{const size=input.dataset.tubeStock.trim(),available=document.querySelector(`[data-tube-available="${CSS.escape(size)}"]`);return [size,{stock:Math.max(0,Number(input.value||0)),available:!!available?.checked}]});const sizes=Object.fromEntries(sortAdminSizes(entries));const product={...DATA.tubeTop,name:'Spandex Tube Top',price:Number(document.getElementById('tubeTopPriceAdmin')?.value||64),compareAtPrice:Number(document.getElementById('tubeTopCompareAtPriceAdmin')?.value||0)||null,color:'Black',description:document.getElementById('tubeTopDescriptionAdmin')?.value||'Double lined and stretchy',sizes,updatedAt:new Date().toISOString()};delete product.sizeGuide;delete product.sizeDescription;await BFStore.setDoc('products/spandexTubeTop',product,false);DATA.tubeTop=product;renderTubeTopInventory();renderInventorySummary();BF.toast('Spandex Tube Top stock saved.');},'Saving tube top stock…')}

function renderProducts(){
  document.getElementById('retailPrice').value=DATA.settings.retailPrice||10;document.getElementById('retailCompareAtPrice').value=Number(DATA.settings.retailCompareAtPrice||0)||'';document.getElementById('twistedRetailPrice').value=DATA.settings.twistedRetailPrice??DATA.settings.retailPrice??10;document.getElementById('twistedRetailCompareAtPrice').value=Number(DATA.settings.twistedRetailCompareAtPrice||0)||'';document.getElementById('lowStockThreshold').value=DATA.settings.lowStockThreshold||10;document.getElementById('smoothAvailable').value=String(DATA.settings.smoothAvailable!==false);document.getElementById('smoothFlatAvailable').value=String(DATA.settings.smoothFlatAvailable!==false);document.getElementById('smoothTwistedAvailable').value=String(DATA.settings.smoothTwistedAvailable!==false);document.getElementById('ribbedAvailable').value=String(DATA.settings.ribbedAvailable===true);document.getElementById('matchingSetsAvailable').value=String(DATA.settings.matchingSetsAvailable===true);document.getElementById('ribbedPrice').value=Number(DATA.settings.ribbedPrice||0)||'';document.getElementById('smoothSetPrice').value=Number(DATA.settings.smoothSetPrice||0)||'';document.getElementById('ribbedSetPrice').value=Number(DATA.settings.ribbedSetPrice||0)||'';
...
function renderWholesale(){
  ...
  document.getElementById('wholesaleFields').innerHTML=groups.map(([label,rows])=>`<div class="wholesale-admin-group-title" style="grid-column:1/-1"><h3>${label}</h3><p>Set what customers pay now, then add an old price only when this bundle is genuinely discounted.</p></div>${rows.map(([p,k,d])=>`<div class="admin-sale-pair"><div class="admin-field sale-current-field"><label>${p} pieces  -  current selling price (GHS)</label><input data-wholesale-key="${k}" type="number" min="0" step="0.01" value="${DATA.settings[k]??d}"><p class="field-help">Customer pays this amount.</p></div><div class="admin-field sale-old-field"><label>Old price before discount (GHS) <span class="optional-label">Optional</span></label><input data-wholesale-key="${k.replace('Price','CompareAtPrice')}" data-compare-for="${k}" type="number" min="0" step="0.01" value="${DATA.settings[k.replace('Price','CompareAtPrice')]??''}" placeholder="Leave blank when not on sale"><p class="field-help">Must be higher than the current price to show crossed out.</p></div></div>`).join('')}`).join('');
}
async function saveWholesale(){
  return withAdminLoading(async()=>{const data={};document.querySelectorAll('[data-wholesale-key]').forEach(i=>data[i.dataset.wholesaleKey]=i.value===''?null:Number(i.value));await BFStore.setDoc('settings/store',data);await BFStore.log('Wholesale pricing updated',data);BF.toast('Wholesale prices saved');await loadAll()},'Saving wholesale prices…');
}
...
function renderCatalogProducts(){
  ...
  const price=document.getElementById('ribbedRetailPriceTab'),compare=document.getElementById('ribbedCompareAtPriceTab');if(price)price.value=Number(DATA.settings.ribbedPrice||0)||'';if(compare)compare.value=Number(DATA.settings.ribbedCompareAtPrice||0)||'';
...
function renderStudioProductList(){const box=document.getElementById('studioProducts');if(!box)return;const q=String(document.getElementById('studioProductSearch')?.value||'').toLowerCase(),cat=document.getElementById('studioProductFilter')?.value||'all',cats=Object.fromEntries(studioLiveCategories().map(c=>[c.id,c]));const products=studioLiveProducts().filter(p=>(cat==='all'||p.category===cat)&&(!q||`${p.name} ${p.subtitle||''} ${cats[p.category]?.name||''}`.toLowerCase().includes(q))).sort((a,b)=>String(a.category).localeCompare(String(b.category))||Number(a.featuredOrder||99)-Number(b.featuredOrder||99));box.innerHTML=products.map(p=>`<article class="studio-product-card"><img src="${escapeAdminValue(BFCatalog.image(p))}" alt=""><div class="studio-product-copy"><small>${escapeAdminValue(cats[p.category]?.name||p.category||'Uncategorised')}</small><strong>${escapeAdminValue(p.name)}</strong><span>${p.available===false?'Hidden / sold out':(BFCatalog.isPreorder(p)?'Pre-order':'Live')} · ${p.price==null?'Category price':BF.money(p.price)}${BFCatalog.isWholesale(p)?' · Wholesale':''}</span></div><button class="small-btn" type="button" onclick="openProductStudio('${escapeAdminValue(p.id)}')">Manage</button></article>`).join('')||'<div class="studio-empty">No products match this view.</div>'}
...
function studioSizeRowHtml(name='',stock=0,available=true){
  return `<div class="studio-size-entry"><div class="admin-field"><label>Size</label><input data-studio-size-name value="${escapeAdminValue(name)}" placeholder="e.g. M" aria-label="Size name"></div><div class="admin-field"><label>Quantity</label><input data-studio-size-stock type="number" min="0" step="1" value="${Math.max(0,Number(stock||0))}" aria-label="Quantity for ${escapeAdminValue(name||'size')}"></div><label class="studio-size-sell"><input data-studio-size-available type="checkbox" ${available?'checked':''}><span>Available</span></label><button class="studio-size-remove" type="button" onclick="removeStudioSizeRow(this)" aria-label="Remove this size"><i class="fa-solid fa-trash-can"></i></button></div>`;
}
function addStudioSizeRow(name=''){
  const box=document.getElementById('studioSizeRows');if(!box)return;
  const existing=[...box.querySelectorAll('[data-studio-size-name]')].map(x=>x.value.trim().toLowerCase());
  if(name&&existing.includes(String(name).toLowerCase())){box.querySelectorAll('[data-studio-size-name]')[existing.indexOf(String(name).toLowerCase())]?.focus();return;}
  box.insertAdjacentHTML('beforeend',studioSizeRowHtml(name,0,true));
  const last=box.lastElementChild?.querySelector('[data-studio-size-name]');if(last&&!name)last.focus();
  syncStudioSizeEmptyState();
}
function removeStudioSizeRow(button){button?.closest('.studio-size-entry')?.remove();syncStudioSizeEmptyState()}
function syncStudioSizeEmptyState(){const box=document.getElementById('studioSizeRows'),empty=document.getElementById('studioSizeEmpty');if(empty)empty.hidden=!!box?.querySelector('.studio-size-entry')}
function studioPreorderSizeObject(){const sizes=['2XS','XS','S','M','L','XL','2XL'];return Object.fromEntries(sizes.map(size=>[size,{stock:0,available:true}]));}
function collectStudioSizes(){
  if(document.getElementById('studioFulfilmentType')?.value==='preorder')return studioPreorderSizeObject();
  const out={};
  document.querySelectorAll('#studioSizeRows .studio-size-entry').forEach(row=>{const name=row.querySelector('[data-studio-size-name]')?.value.trim();if(!name)return;out[name]={stock:Math.max(0,Number(row.querySelector('[data-studio-size-stock]')?.value||0)),available:!!row.querySelector('[data-studio-size-available]')?.checked}});
  return Object.fromEntries(sortAdminSizes(Object.entries(out)));
}
window.addStudioSizeRow=addStudioSizeRow;window.removeStudioSizeRow=removeStudioSizeRow;
function studioWholesaleTierRowsHtml(wholesale={}){
  const tiers=Array.isArray(wholesale.tiers)&&wholesale.tiers.length?wholesale.tiers:[{minQty:Number(wholesale.minQty||10),price:''}];
  return tiers.map((tier,index)=>`<div class="studio-wholesale-tier-row" data-wholesale-tier><div class="admin-field"><label>Minimum units</label><input data-wholesale-min type="number" min="1" step="1" value="${Number(tier.minQty||1)}"></div><div class="admin-field"><label>Price per unit (GHS)</label><input data-wholesale-price type="number" min="0" step="0.01" value="${tier.price??''}" placeholder="0.00"></div><button type="button" class="small-btn danger studio-wholesale-remove" onclick="removeStudioWholesaleTier(this)" aria-label="Remove wholesale price tier"><i class="fa-solid fa-trash"></i></button></div>`).join('');
}
function addStudioWholesaleTier(){
  const box=document.getElementById('studioWholesaleTiers');if(!box)return;
  const last=[...box.querySelectorAll('[data-wholesale-tier]')].at(-1);
  const min=Math.max(1,Number(last?.querySelector('[data-wholesale-min]')?.value||10)+10);
  box.insertAdjacentHTML('beforeend',`<div class="studio-wholesale-tier-row" data-wholesale-tier><div class="admin-field"><label>Minimum units</label><input data-wholesale-min type="number" min="1" step="1" value="${min}"></div><div class="admin-field"><label>Price per unit (GHS)</label><input data-wholesale-price type="number" min="0" step="0.01" placeholder="0.00"></div><button type="button" class="small-btn danger studio-wholesale-remove" onclick="removeStudioWholesaleTier(this)" aria-label="Remove wholesale price tier"><i class="fa-solid fa-trash"></i></button></div>`);
  box.querySelectorAll('[data-wholesale-tier] input').at(-1)?.focus();
}
function removeStudioWholesaleTier(button){
  const rows=document.querySelectorAll('#studioWholesaleTiers [data-wholesale-tier]');
  if(rows.length<=1)return BF.toast('Keep at least one wholesale price tier.');
  button?.closest('[data-wholesale-tier]')?.remove();
}
function collectStudioWholesale(){
  const enabled=document.getElementById('studioWholesaleEnabled')?.value==='true';
  if(!enabled)return {enabled:false,minQty:1,tiers:[]};
  const tiers=[...document.querySelectorAll('#studioWholesaleTiers [data-wholesale-tier]')].map(row=>({minQty:Math.max(1,Math.floor(Number(row.querySelector('[data-wholesale-min]')?.value||0))),price:Number(row.querySelector('[data-wholesale-price]')?.value||0)})).filter(x=>x.minQty>0&&x.price>0).sort((a,b)=>a.minQty-b.minQty);
  if(!tiers.length)throw new Error('Add at least one wholesale price tier.');
  const seen=new Set();for(const tier of tiers){if(seen.has(tier.minQty))throw new Error(`Wholesale minimum ${tier.minQty} is repeated. Use each minimum only once.`);seen.add(tier.minQty);}
  return {enabled:true,minQty:tiers[0].minQty,tiers};
}
function syncStudioCommercialFields(){
  const enabled=document.getElementById('studioWholesaleEnabled')?.value==='true';
  const ribbed=document.getElementById('studioProductCategory')?.value==='ribbed';
  const wholesaleField=document.getElementById('studioWholesaleEnabled');
  if(wholesaleField) wholesaleField.disabled=ribbed;
  const settings=document.getElementById('studioWholesaleSettings');
  settings?.toggleAttribute('hidden',!enabled||ribbed);
  const help=document.querySelector('#studioWholesaleSettings')?.previousElementSibling?.querySelector('.field-help');
  if(help) help.textContent=ribbed?'Hairband wholesale is managed from the dedicated Hairbands Wholesale page.':'When enabled, this product appears on Other Products Wholesale.';
  const preorder=document.getElementById('studioFulfilmentType')?.value==='preorder';
  document.getElementById('studioPreorderDateWrap')?.toggleAttribute('hidden',!preorder);
}
window.addStudioWholesaleTier=addStudioWholesaleTier;window.removeStudioWholesaleTier=removeStudioWholesaleTier;

function studioVariantSizeRowHtml(variantIndex,size='',stock=0,available=true){
  return `<div class="studio-variant-size-entry" data-variant-size><div class="admin-field"><label>Size</label><input data-variant-size-name value="${escapeAdminValue(size)}" placeholder="e.g. M"></div><div class="admin-field"><label>Quantity</label><input data-variant-size-stock type="number" min="0" step="1" value="${Math.max(0,Number(stock||0))}"></div><label class="studio-size-sell"><input data-variant-size-available type="checkbox" ${available?'checked':''}><span>Available</span></label><button type="button" class="studio-size-remove" onclick="removeStudioVariantSizeRow(this)" aria-label="Remove this size"><i class="fa-solid fa-trash-can"></i></button></div>`;
}
function studioVariantImageList(rowOrVariant={}){
  if(rowOrVariant?.nodeType===1){
    try{const parsed=JSON.parse(rowOrVariant.querySelector('[data-variant-images]')?.value||'[]');return Array.isArray(parsed)?parsed.filter(Boolean):[];}catch{return [];}
  }
  const v=rowOrVariant||{};const list=Array.isArray(v.images)?v.images.filter(Boolean):[];if(v.image&&!list.includes(v.image))list.unshift(v.image);return [...new Set(list)];
}
function studioVariantPhotosHtml(images=[]){
  if(!images.length)return '<div class="studio-variant-empty-photos"><i class="fa-regular fa-images"></i><span>Add one or more photos for this colour.</span></div>';
  return images.map((url,i)=>`<div class="studio-variant-photo-item"><img src="${escapeAdminValue(url)}" alt=""><span>${i===0?'Main photo':'Photo '+(i+1)}</span><button type="button" onclick="removeStudioVariantImage(this)" aria-label="Remove photo"><i class="fa-solid fa-xmark"></i></button></div>`).join('');
}
function renderStudioVariantPhotos(row){
  const images=studioVariantImageList(row),input=row?.querySelector('[data-variant-images]'),primary=row?.querySelector('[data-variant-image]'),grid=row?.querySelector('[data-variant-photo-grid]');
  if(input)input.value=JSON.stringify(images);if(primary)primary.value=images[0]||'';if(grid)grid.innerHTML=studioVariantPhotosHtml(images);
}
function addStudioVariantImage(row,url){if(!row||!url)return;const images=studioVariantImageList(row);if(!images.includes(url))images.push(url);const input=row.querySelector('[data-variant-images]');if(input)input.value=JSON.stringify(images);row.querySelector('[data-variant-image]')?.setAttribute('value',images[0]||'');if(row.querySelector('[data-variant-image]'))row.querySelector('[data-variant-image]').value=images[0]||'';const grid=row.querySelector('[data-variant-photo-grid]');if(grid)grid.innerHTML=studioVariantPhotosHtml(images);}
function removeStudioVariantImage(button){const row=button?.closest('[data-variant-row]');if(!row)return;const item=button.closest('.studio-variant-photo-item'),index=[...row.querySelectorAll('.studio-variant-photo-item')].indexOf(item);const images=studioVariantImageList(row);if(index<0)return;images.splice(index,1);const input=row.querySelector('[data-variant-images]');if(input)input.value=JSON.stringify(images);if(row.querySelector('[data-variant-image]'))row.querySelector('[data-variant-image]').value=images[0]||'';const grid=row.querySelector('[data-variant-photo-grid]');if(grid)grid.innerHTML=studioVariantPhotosHtml(images);}
async function uploadStudioVariantAssets(input,row){
  const files=[...(input?.files||[])].filter(file=>String(file.type||'').startsWith('image/'));
  if(!files.length||!row)return;
  input.disabled=true;
  try{
    let added=0;
    for(const file of files){const url=await uploadStudioFile(file,'','',false,false,row.querySelector('.studio-variant-photo'),false);if(url){addStudioVariantImage(row,url);added++;}}
    if(added)BF.toast(`${added} photo${added===1?'':'s'} added. Save your changes when you are done.`);
  }finally{input.disabled=false;input.value='';}
}
window.uploadStudioVariantAssets=uploadStudioVariantAssets;
function studioVariantRowHtml(variant={},index=0){
  const sizes=variant.sizes||{},images=studioVariantImageList(variant),fallbackImage=images[0]||'';
  return `<article class="studio-variant-row" data-variant-row data-variant-index="${index}"><div class="studio-variant-row-head"><div class="studio-variant-row-title"><span class="studio-variant-number">Colour ${index+1}</span><strong data-variant-title>${escapeAdminValue(variant.color||`Colour ${index+1}`)}</strong></div><button type="button" class="small-btn danger" onclick="removeStudioVariantRow(this)"><i class="fa-solid fa-trash"></i> Remove</button></div><div class="studio-variant-main"><div class="studio-variant-photo"><div class="studio-variant-photo-grid" data-variant-photo-grid>${studioVariantPhotosHtml(images)}</div><input type="hidden" data-variant-image value="${escapeAdminValue(fallbackImage)}"><input type="hidden" data-variant-images value="${escapeAdminValue(JSON.stringify(images))}"><div class="studio-variant-photo-actions"><label class="small-btn pink">Add photos<input hidden type="file" accept="image/*" multiple onchange="uploadStudioVariantAssets(this,this.closest('[data-variant-row]'))"></label><span>First photo = this colour's shop image</span></div></div><div class="studio-form-grid studio-variant-fields"><div class="admin-field"><label>Colour name</label><input data-variant-color value="${escapeAdminValue(variant.color||'')}" placeholder="e.g. Chocolate Brown" required></div><div class="admin-field"><label>Swatch colour</label><input data-variant-hex type="color" value="${escapeAdminValue(variant.hex||'#d9d9d9')}" aria-label="Swatch colour"></div><div class="admin-field"><label>Colour availability</label><select data-variant-available><option value="true" ${variant.available===false?'':'selected'}>Available</option><option value="false" ${variant.available===false?'selected':''}>Not available</option></select></div></div></div><div class="studio-variant-sizes"><div class="studio-variant-sizes-head"><div><strong>Sizes and quantities</strong><small>Stock is separate for every colour and size. Pre-orders automatically use 2XS through 2XL.</small></div><div class="studio-size-quick">${['2XS','XS','S','M','L','XL','2XL'].map(size=>`<button type="button" onclick="addStudioVariantSizeRow(${index},'${size}')">${size}</button>`).join('')}<button class="custom" type="button" onclick="addStudioVariantSizeRow(${index},'')"><i class="fa-solid fa-plus"></i> Custom</button></div></div><div class="studio-variant-size-rows" data-variant-size-rows>${Object.entries(sizes).map(([size,d])=>studioVariantSizeRowHtml(index,size,Number(d?.stock||0),d?.available!==false)).join('')}</div><button type="button" class="small-btn studio-add-size" onclick="addStudioVariantSizeRow(${index},'')"><i class="fa-solid fa-plus"></i> Add another size</button></div></article>`;
}
function studioVariantRowsHtml(variants=[]){const list=Array.isArray(variants)?variants:[];return (list.length?list:[{}]).map((v,i)=>studioVariantRowHtml(v,i)).join('');}
function addStudioVariantRow(seed={}){const box=document.getElementById('studioVariantRows');if(!box)return;const index=box.querySelectorAll('[data-variant-row]').length;box.insertAdjacentHTML('beforeend',studioVariantRowHtml(seed,index));syncStudioFeaturedVariantSelect(document.getElementById('studioFeaturedVariant')?.value||'');box.lastElementChild?.querySelector('[data-variant-color]')?.focus();}
function removeStudioVariantRow(button){const rows=document.querySelectorAll('#studioVariantRows [data-variant-row]');if(rows.length<=1)return BF.toast('Keep at least one colour.');button?.closest('[data-variant-row]')?.remove();renumberStudioVariantRows();syncStudioFeaturedVariantSelect(document.getElementById('studioFeaturedVariant')?.value||'');}
function renumberStudioVariantRows(){document.querySelectorAll('#studioVariantRows [data-variant-row]').forEach((row,index)=>{row.dataset.variantIndex=index;row.querySelector('.studio-variant-row-head span')?.replaceChildren(document.createTextNode(`Colour ${index+1}`));row.querySelector('[data-variant-title]')?.replaceChildren(document.createTextNode(row.querySelector('[data-variant-color]')?.value.trim()||`Colour ${index+1}`));row.querySelectorAll('.studio-size-quick button[onclick]').forEach(btn=>{const current=btn.getAttribute('onclick')||'';btn.setAttribute('onclick',current.replace(/addStudioVariantSizeRow\(\d+/,`addStudioVariantSizeRow(${index}`));});row.querySelector('.studio-add-size')?.setAttribute('onclick',`addStudioVariantSizeRow(${index},'')`);});}
function addStudioVariantSizeRow(variantIndex,name=''){const rows=[...document.querySelectorAll('#studioVariantRows [data-variant-row]')],row=rows[Number(variantIndex)],box=row?.querySelector('[data-variant-size-rows]');if(!box)return;const existing=[...box.querySelectorAll('[data-variant-size-name]')].map(x=>x.value.trim().toLowerCase());if(name&&existing.includes(String(name).toLowerCase()))return;box.insertAdjacentHTML('beforeend',studioVariantSizeRowHtml(variantIndex,name,0,true));if(!name)box.lastElementChild?.querySelector('[data-variant-size-name]')?.focus();}
function removeStudioVariantSizeRow(button){button?.closest('[data-variant-size]')?.remove();}
function collectStudioVariants(){
  const rows=[...document.querySelectorAll('#studioVariantRows [data-variant-row]')];if(!rows.length)throw new Error('Add at least one colour variant.');
  const seen=new Set(),variants=[];
  const preorder=document.getElementById('studioFulfilmentType')?.value==='preorder';
  rows.forEach((row,index)=>{const color=row.querySelector('[data-variant-color]')?.value.trim()||'';if(!color)throw new Error(`Add a colour name for Colour ${index+1}.`);const key=color.toLowerCase();if(seen.has(key))throw new Error(`${color} is repeated. Use each colour only once.`);seen.add(key);const images=studioVariantImageList(row),sizes=preorder?studioPreorderSizeObject():{};if(!preorder)row.querySelectorAll('[data-variant-size]').forEach(sizeRow=>{const name=sizeRow.querySelector('[data-variant-size-name]')?.value.trim();if(!name)return;sizes[name]={stock:Math.max(0,Number(sizeRow.querySelector('[data-variant-size-stock]')?.value||0)),available:!!sizeRow.querySelector('[data-variant-size-available]')?.checked};});const idBase=studioSlug(color)||`variant-${index+1}`;variants.push({id:idBase,color,hex:row.querySelector('[data-variant-hex]')?.value||'#d9d9d9',image:images[0]||'',images,sizes,available:row.querySelector('[data-variant-available]')?.value!=='false'});});
  const ids=new Set();variants.forEach(v=>{let id=v.id,i=2;while(ids.has(id)){id=`${v.id}-${i++}`;}v.id=id;ids.add(id);});return variants;
}
window.addStudioVariantRow=addStudioVariantRow;window.removeStudioVariantRow=removeStudioVariantRow;window.addStudioVariantSizeRow=addStudioVariantSizeRow;window.removeStudioVariantSizeRow=removeStudioVariantSizeRow;window.removeStudioVariantImage=removeStudioVariantImage;
function syncStudioFeaturedVariantSelect(current=''){
  const select=document.getElementById('studioFeaturedVariant'),rows=[...document.querySelectorAll('#studioVariantRows [data-variant-row]')];if(!select)return;
  const previous=current||select.value||select.dataset.current||'';
  select.innerHTML=rows.map((row,i)=>{const name=row.querySelector('[data-variant-color]')?.value.trim()||`Colour ${i+1}`,id=studioSlug(name)||`variant-${i+1}`;return `<option value="${escapeAdminValue(id)}">${escapeAdminValue(name)}</option>`}).join('');
...
function openProductStudio(id=''){
  ...
  openStudioModal(`<form id="studioProductForm" class="studio-form"><div class="studio-form-head"><span>Product editor</span><h2>${isNew?'Add a new product':'Edit product'}</h2><p>Set the shared product details once, then add each colour below. Each colour is part of this one product. The default colour is shown on shop, home and wholesale cards until the customer chooses another colour.</p></div><div class="studio-cover-edit product"><img id="studioProductPreview" src="${escapeAdminValue(existing.image||BFCatalog.image(existing)||'images/placeholder-product.svg')}" alt=""><div class="studio-upload-zone"><strong>Product image</strong><p>This is a fallback image. Colour photos below will be used first when variants are set.</p><label class="small-btn pink">Choose image<input hidden type="file" accept="image/*" onchange="uploadStudioAsset(this,'studioProductPreview','studioProductImage')"></label></div></div><input id="studioProductImage" type="hidden" value="${escapeAdminValue(existing.image||'')}"><input id="studioProductImages" type="hidden" value="${escapeAdminValue(JSON.stringify(existing.images||[]))}"><div class="studio-gallery-editor"><div><strong>Extra product images</strong><p>Optional shared images for the product page.</p></div><label class="small-btn">+ Add image<input hidden type="file" accept="image/*" onchange="uploadStudioAsset(this,'','',false,true)"></label><div id="studioGalleryList" class="studio-gallery-list"></div></div><div class="studio-form-grid"><div class="admin-field"><label>Product name</label><input id="studioProductName" required value="${escapeAdminValue(existing.name||'')}" placeholder="Product name"></div><div class="admin-field"><label>Category</label><select id="studioProductCategory">${categories.map(c=>`<option value="${escapeAdminValue(c.id)}" ${existing.category===c.id?'selected':''}>${escapeAdminValue(c.name)}</option>`).join('')}</select></div><div class="admin-field sale-current-field"><label>Current selling price (GHS)</label><input id="studioProductPrice" type="number" min="0" step="0.01" value="${existing.price??''}" placeholder="0.00"><p class="field-help">What the customer pays now.</p></div><div class="admin-field sale-old-field"><label>Old price before discount (GHS) <span class="optional-label">Optional</span></label><input id="studioProductCompareAtPrice" type="number" min="0" step="0.01" value="${existing.compareAtPrice??''}" placeholder="Leave blank when not on sale"><p class="field-help">Only enter a higher former price when this item is on sale.</p></div><div class="admin-field"><label>Product availability</label><select id="studioProductAvailable"><option value="true" ${existing.available!==false?'selected':''}>Available to buy</option><option value="false" ${existing.available===false?'selected':''}>Not available</option></select></div><div class="admin-field"><label>Short detail</label><input id="studioProductSubtitle" value="${escapeAdminValue(existing.subtitle||'')}" placeholder="e.g. Fitted maxi skirt"></div><div class="admin-field"><label>Display order</label><input id="studioProductOrder" type="number" min="1" value="${Number(existing.featuredOrder||99)}"></div><div class="admin-field span-2"><label>Description</label><textarea id="studioProductDescription" placeholder="Tell customers about this product">${escapeAdminValue(existing.description||'')}</textarea></div></div><section class="studio-variant-toggle"><label class="studio-check-card"><span><input id="studioVariantMode" type="checkbox" ${variantMode?'checked':''}><span><strong>Use colour variants</strong><small>Use this when the same product comes in different colours. Each colour can have its own photo, sizes and stock.</small></span></span><b>Recommended for new products</b></label></section><section id="studioVariantBuilder" class="studio-variant-builder" ${variantMode?'':'hidden'}><div class="studio-section-head"><div><strong>Colours, photos and stock</strong><span>Add the colours for this one product. Each colour can have its own photos, sizes and stock.</span></div><button type="button" class="small-btn" onclick="addStudioVariantRow()"><i class="fa-solid fa-plus"></i> Add colour</button></div><div class="studio-featured-variant-row"><div><strong>Default storefront colour</strong><small>This is the colour shown first on shop, home and wholesale cards.</small></div><select id="studioFeaturedVariant"></select></div><div id="studioVariantRows">${studioVariantRowsHtml(existing.variants||[])}</div></section><details id="studioSingleSizeGroup" class="studio-detail-group studio-single-size-block" ${variantMode?'hidden':''} open><summary><span><strong>Sizes and quantities</strong><small>Use this for products that do not need separate colour variants. Pre-orders automatically use 2XS through 2XL.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body"><div class="studio-size-quick"><span>Quick add</span>${['2XS','XS','S','M','L','XL','2XL'].map(size=>`<button type="button" onclick="addStudioSizeRow('${size}')">${size}</button>`).join('')}<button class="custom" type="button" onclick="addStudioSizeRow('')"><i class="fa-solid fa-plus"></i> Custom size</button></div><div id="studioSizeRows" class="studio-size-rows">${studioSizeRowsHtml(existing.sizes||{})}</div><div id="studioSizeEmpty" class="studio-size-empty" ${Object.keys(existing.sizes||{}).length?'hidden':''}><i class="fa-solid fa-ruler-combined"></i><strong>No sizes added</strong><span>Add sizes only when the product needs them.</span></div><button class="small-btn studio-add-size" type="button" onclick="addStudioSizeRow('')"><i class="fa-solid fa-plus"></i> Add another size</button></div></details><details class="studio-detail-group"><summary><span><strong>Stock and display extras</strong><small>Use these only for products without colour variants.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body studio-form-grid"><div class="admin-field studio-simple-stock"><label>One-size quantity</label><input id="studioProductStock" type="number" min="0" value="${Number(existing.stock||0)}"><p class="field-help">For products that do not use sizes.</p></div><div class="admin-field"><label>Pack size</label><input id="studioProductPack" type="number" min="1" value="${Number(existing.packSize||1)}"></div><div class="studio-ribbed-fields span-2"><strong>Ribbed style quantities</strong><p>Only used for Ribbed products with separate Flat and Twisted stock.</p><div class="studio-ribbed-grid"><div class="admin-field"><label>Flat quantity</label><input id="studioRibbedFlatStock" type="number" min="0" value="${Number(existing.styles?.flat?.stock??existing.stock??0)}"></div><div class="admin-field"><label>Flat availability</label><select id="studioRibbedFlatAvailable"><option value="true" ${existing.styles?.flat?.available===false?'':'selected'}>Available</option><option value="false" ${existing.styles?.flat?.available===false?'selected':''}>Not available</option></select></div><div class="admin-field"><label>Twisted quantity</label><input id="studioRibbedTwistedStock" type="number" min="0" value="${Number(existing.styles?.twisted?.stock??0)}"></div><div class="admin-field"><label>Twisted availability</label><select id="studioRibbedTwistedAvailable"><option value="true" ${existing.styles?.twisted?.available===false?'':'selected'}>Available</option><option value="false" ${existing.styles?.twisted?.available===false?'selected':''}>Not available</option></select></div></div></div></div></details><details class="studio-detail-group" open><summary><span><strong>Wholesale</strong><small>Control whether this product appears in the Other Products Wholesale shop.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body"><div class="studio-form-grid"><div class="admin-field"><label>Available for wholesale</label><select id="studioWholesaleEnabled"><option value="false" ${wholesale.enabled?'':'selected'}>No</option><option value="true" ${wholesale.enabled?'selected':''}>Yes</option></select><p class="field-help">Enable this for generic products. Hairband wholesale stays on the dedicated Hairbands Wholesale page.</p></div></div><div id="studioWholesaleSettings" ${wholesale.enabled?'':'hidden'}><div class="studio-wholesale-tier-head"><div><strong>Wholesale price tiers</strong><span>Use one tier for a single wholesale price, or add more tiers for quantity-based pricing.</span></div><button type="button" class="small-btn" onclick="addStudioWholesaleTier()"><i class="fa-solid fa-plus"></i> Add price tier</button></div><div id="studioWholesaleTiers" class="studio-wholesale-tier-rows">${studioWholesaleTierRowsHtml(wholesale)}</div></div></div></details><details class="studio-detail-group"><summary><span><strong>Fulfilment</strong><small>Set this to Pre-order when customers can pay before the item is ready.</small></span><i class="fa-solid fa-chevron-down"></i></summary><div class="studio-detail-body studio-form-grid"><div class="admin-field"><label>Fulfilment type</label><select id="studioFulfilmentType"><option value="standard" ${fulfilment.type==='standard'?'selected':''}>Standard / ready stock</option><option value="preorder" ${fulfilment.type==='preorder'?'selected':''}>Pre-order</option></select><p class="field-help">Pre-order products stay purchasable even when current stock is zero. For pre-orders, sizes 2XS through 2XL are added automatically and quantity does not need to be entered.</p></div><div class="admin-field" id="studioPreorderDateWrap" ${fulfilment.type==='preorder'?'':'hidden'}><label>Pre-order fulfilment date</label><input id="studioPreorderDate" type="date" value="${escapeAdminValue(fulfilment.date||'')}"><p class="field-help">This date will be shown on the product, bag, checkout, emails and order tracking.</p></div></div></details><div class="studio-form-actions">${!isNew?'<button class="small-btn danger" type="button" id="deleteStudioProduct">Remove product</button>':''}<span></span><button class="small-btn" type="button" onclick="closeStudioModal()">Cancel</button><button class="small-btn primary" type="submit"><i class="fa-solid fa-check"></i> Save product</button></div></form>`);
  document.getElementById('studioProductForm').onsubmit=e=>saveStudioProduct(e,id);document.getElementById('deleteStudioProduct')?.addEventListener('click',()=>deleteStudioProduct(id));
  document.getElementById('studioVariantRows')?.addEventListener('input',e=>{if(!e.target.matches('[data-variant-color]'))return;const row=e.target.closest('[data-variant-row]');row?.querySelector('[data-variant-title]')?.replaceChildren(document.createTextNode(e.target.value.trim()||`Colour ${(Number(row?.dataset.variantIndex||0)+1)}`));}); const featuredVariantId=existing.featuredVariantId||((existing.variants||[]).find(v=>String(v.color||'')===String(existing.featuredColor||existing.featuredColour||''))?.id)||'';syncStudioFeaturedVariantSelect(featuredVariantId);document.getElementById('studioFeaturedVariant')?.addEventListener('change',e=>e.target.dataset.current=e.target.value);document.getElementById('studioVariantRows')?.addEventListener('input',e=>{if(!e.target.matches('[data-variant-color]'))return;const row=e.target.closest('[data-variant-row]');row?.querySelector('[data-variant-title]')?.replaceChildren(document.createTextNode(e.target.value.trim()||`Colour ${(Number(row?.dataset.variantIndex||0)+1)}`));syncStudioFeaturedVariantSelect(document.getElementById('studioFeaturedVariant')?.dataset.current||'');});
...
async function uploadStudioFile(file,previewId,urlId,backgroundPreview=false,galleryUpload=false,zone=null,announce=true){
  ...
  try{
    ...
    if(announce)BF.toast('Image uploaded. Save your changes when you are done.');
