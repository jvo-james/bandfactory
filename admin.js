...
function studioVariantRowHtml(variant={},index=0){
  ...
  return `<article class="studio-variant-row" data-variant-row data-variant-index="${index}">
    <div class="studio-variant-row-head"><div><span>Colour ${index+1}</span><strong data-variant-title>${escapeAdminValue(variant.color||`Colour ${index+1}`)}</strong></div><button type="button" class="small-btn danger" onclick="removeStudioVariantRow(this)"><i class="fa-solid fa-trash"></i> Remove colour</button></div>
    <div class="studio-variant-main">
      <div class="studio-variant-photo">
        <div class="studio-variant-photo-grid" data-variant-photo-grid>${studioVariantPhotosHtml(images)}</div>
        <input type="hidden" data-variant-image value="${escapeAdminValue(fallbackImage)}">
        <input type="hidden" data-variant-images value="${escapeAdminValue(JSON.stringify(images))}">
        <label class="small-btn pink">Add photos<input hidden type="file" accept="image/*" multiple onchange="uploadStudioVariantAssets(this,this.closest('[data-variant-row]'))"></label>
        <small>The first photo is used on the shop card. Add more photos for the product page.</small>
      </div>
      <div class="studio-form-grid studio-variant-fields"><div class="admin-field"><label>Colour name</label><input data-variant-color value="${escapeAdminValue(variant.color||'')}" placeholder="e.g. Chocolate Brown" required></div><div class="admin-field"><label>Colour availability</label><select data-variant-available><option value="true" ${variant.available===false?'':'selected'}>Available</option><option value="false" ${variant.available===false?'selected':''}>Not available</option></select></div></div>
    </div>
    <div class="studio-variant-sizes"><div class="studio-variant-sizes-head"><div><strong>Sizes and quantities for this colour</strong><small>Enter the stock separately for each size.</small></div><div class="studio-size-quick">${['XS','S','M','L','XL','2XL'].map(size=>`<button type="button" onclick="addStudioVariantSizeRow(${index},'${size}')">${size}</button>`).join('')}<button class="custom" type="button" onclick="addStudioVariantSizeRow(${index},'')"><i class="fa-solid fa-plus"></i> Custom</button></div></div><div class="studio-variant-size-rows" data-variant-size-rows>${Object.entries(sizes).map(([size,d])=>studioVariantSizeRowHtml(index,size,Number(d?.stock||0),d?.available!==false)).join('')}</div><button type="button" class="small-btn studio-add-size" onclick="addStudioVariantSizeRow(${index},'')"><i class="fa-solid fa-plus"></i> Add another size</button></div>
  </article>`;
}
...
function addStudioVariantRow(seed={}){const box=document.getElementById('studioVariantRows');if(!box)return;const index=box.querySelectorAll('[data-variant-row]').length;box.insertAdjacentHTML('beforeend',studioVariantRowHtml(seed,index));box.lastElementChild?.querySelector('[data-variant-color]')?.focus();}
function removeStudioVariantRow(button){const rows=document.querySelectorAll('#studioVariantRows [data-variant-row]');if(rows.length<=1)return BF.toast('Keep at least one colour.');button?.closest('[data-variant-row]')?.remove();renumberStudioVariantRows();}
...
function openProductStudio(id=''){
  ...
  document.getElementById('studioVariantRows')?.addEventListener('input',e=>{if(!e.target.matches('[data-variant-color]'))return;const row=e.target.closest('[data-variant-row]');row?.querySelector('[data-variant-title]')?.replaceChildren(document.createTextNode(e.target.value.trim()||`Colour ${(Number(row?.dataset.variantIndex||0)+1)}`));});
...
async function saveStudioProduct(e,id){
  ...
  return withAdminLoading(async()=>{
    ...
    const variantMode=category!=='ribbed'&&document.getElementById('studioVariantMode')?.checked;if(variantMode){item.variants=variants;delete item.sizes;delete item.styles;const first=variants.find(v=>v.image)?.image||item.image;if(first)item.image=first;item.color='';}else{delete item.variants;if(Object.keys(sizes).length)item.sizes=sizes;else delete item.sizes;}
