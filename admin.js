...
let loadingDepth=0;
const DISMISSED_ALERTS_KEY='bandFactoryAdminDismissedAlerts';
...
function renderAll(){
  renderNavCounts();renderOverview();renderAnalytics();renderOrders();renderTransactions();renderProducts();renderInventorySummary();renderWholesale();renderReviews();renderCustomers();renderAbandonedCarts();renderSubscribers();renderDelivery();renderMessages();renderNotifications();renderTopAlerts();renderActivity();renderSettings();
}
...
function renderNavCounts(){
  ...
  setNavCount('ordersNavCount',pendingOrders);setNavCount('reviewsNavCount',pendingReviews);setNavCount('messagesNavCount',unreadMessages);setNavCount('notificationsNavCount',unreadNotifications);setNavCount('productsNavCount',lowStock);setNavCount('abandonedNavCount',abandoned);
...
function openOrder(id){
  ...
  document.getElementById('orderDrawer').innerHTML=`
    ...
    <div class="order-drawer-content">
      <div class="order-title-row"><div><h2>${o.id}</h2><p><strong>${o.name||'Customer'}</strong><br>${o.email||'No email provided'}<br>${o.phone||''}</p></div><div class="order-title-badges"><span class="badge paid">${o.payment||'Paid'}</span><span class="badge ${String(o.status).toLowerCase()}">${o.status||'Preparing'}</span></div></div>
      ...
      <div class="admin-field order-status-field"><label>Order status</label><select id="drawerStatus"><option>Preparing</option><option>Ready</option><option>Dispatched</option><option>Delivered</option><option>Cancelled</option></select><p class="field-help">Choose the stage this order has reached, then save.</p></div>
      <div class="drawer-save-row"><button class="small-btn primary" onclick="saveOrderStatus('${o.id}')">Save order status</button></div>
    </div>`;
  document.getElementById('drawerStatus').value=o.status||'Preparing';
...
function notificationRoute(n){
  ...
  if(n.type==='inventory')return {section:'productsPanel',orderId:n.orderId};
...
async function openNotification(id){
  ...
  if(route.orderId){showSection('ordersPanel');openOrder(route.orderId);}
  else if(route.reviewId){showSection('reviewsPanel');requestAnimationFrame(()=>document.querySelector(`[data-review-id="${route.reviewId}"]`)?.scrollIntoView({behavior:'smooth',block:'center'}));}
  else showSection(route.section);
  renderNavCounts();renderTopAlerts();renderNotifications();renderOverview();
}
window.openNotification=openNotification;
function renderNotifications(){
  document.getElementById('notificationsList').innerHTML=DATA.notifications.length?DATA.notifications.map(n=>`<article class="notification-card actionable-notification ${n.read?'':'unread'}" onclick="openNotification('${n.id}')" tabindex="0" role="button"><div class="notification-main"><strong>${n.title}</strong><p>${n.message||''}</p><div class="notification-meta"><small>${fmtDate(n.createdAt)}</small>${n.read?'':'<span class="badge pending">New</span>'}</div></div><i class="fa-solid fa-chevron-right notification-go"></i></article>`).join(''):'<p>No notifications yet.</p>';
}
...
function getDismissedAlerts(){try{return new Set(JSON.parse(localStorage.getItem(DISMISSED_ALERTS_KEY)||'[]'))}catch{return new Set()}}
function dismissTopAlert(id,event){
  event?.stopPropagation();
  const dismissed=getDismissedAlerts();dismissed.add(id);localStorage.setItem(DISMISSED_ALERTS_KEY,JSON.stringify([...dismissed]));
  document.querySelector(`[data-top-alert-id="${id}"]`)?.remove();
  const area=document.getElementById('adminAlertArea');if(area&&!area.querySelector('.top-admin-alert'))area.innerHTML='';
}
window.dismissTopAlert=dismissTopAlert;
function notificationIcon(type){return type==='purchase'?'fa-bag-shopping':type==='review'?'fa-star':type==='message'?'fa-comment-dots':type==='subscriber'?'fa-user-plus':'fa-bell'}
function renderTopAlerts(){
  const area=document.getElementById('adminAlertArea');if(!area)return;
  const dismissed=getDismissedAlerts();
  const visible=DATA.notifications.filter(n=>!n.read&&!dismissed.has(n.id)).slice(0,4);
  if(!visible.length){area.innerHTML='';return}
  area.innerHTML=visible.map(n=>`<article class="top-admin-alert" data-top-alert-id="${n.id}" onclick="openNotification('${n.id}')" tabindex="0" role="button"><div class="top-admin-alert-icon"><i class="fa-solid ${notificationIcon(n.type)}"></i></div><div class="top-admin-alert-copy"><strong>${n.title}</strong><p>${n.message||'Tap to open'}</p><small>${fmtDate(n.createdAt)} · Tap to view</small></div><button type="button" class="top-admin-alert-close" aria-label="Close notification" onclick="dismissTopAlert('${n.id}',event)"><i class="fa-solid fa-xmark"></i></button></article>`).join('')+(DATA.notifications.filter(n=>!n.read&&!dismissed.has(n.id)).length>4?'<button class="top-admin-alert-more" type="button" onclick="showSection(\'notificationsPanel\')">View all notifications</button>':'');
}
...
function renderSettings(){document.getElementById('storeEmail').value=DATA.settings.storeEmail||BF_CONFIG.emailjs.adminEmail;document.getElementById('instagramUrl').value=DATA.settings.instagramUrl||BF_CONFIG.socials.instagram;document.getElementById('tiktokUrl').value=DATA.settings.tiktokUrl||BF_CONFIG.socials.tiktok}
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){closeSidebar();closeOrder()}
  if((e.key==='Enter'||e.key===' ')&&e.target.matches('.clickable-stat')){e.preventDefault();showSection(e.target.dataset.go)}
  if((e.key==='Enter'||e.key===' ')&&e.target.matches('.actionable-notification,.top-admin-alert')){e.preventDefault();e.target.click()}
});
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('headerNotifications').onclick=()=>showSection('notificationsPanel');
  ...
  document.getElementById('orderSearch').oninput=renderOrders;document.getElementById('orderFilter').onchange=renderOrders;document.getElementById('analyticsRange').onchange=renderAnalytics;document.getElementById('abandonedFilter').onchange=renderAbandonedCarts;
