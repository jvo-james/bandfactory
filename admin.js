...
function renderAll(){
  renderNavCounts();renderOverview();renderAnalytics();renderOrders();renderPendingPayments();renderTransactions();renderInternationalPayments();renderProducts();renderCatalogProducts();renderCatalogStudio();renderTubeTopInventory();renderInventorySummary();renderWholesale();renderReviews();renderCustomers();renderAbandonedCarts();renderSubscribers();renderDelivery();renderMessages();renderNotifications();renderNotificationPopover();renderActivity();renderSettings();
}
...
/* =====================================================
   CATALOG STUDIO — categories, products & Cloudinary
===================================================== */
...
document.addEventListener('DOMContentLoaded',()=>{document.getElementById('newCategoryBtn')?.addEventListener('click',()=>openCategoryStudio());document.getElementById('newProductBtn')?.addEventListener('click',()=>openProductStudio());document.getElementById('studioModalClose')?.addEventListener('click',closeStudioModal);document.getElementById('studioModal')?.addEventListener('click',e=>{if(e.target.id==='studioModal')closeStudioModal()});document.getElementById('studioProductFilter')?.addEventListener('change',renderStudioProductList);document.getElementById('studioProductSearch')?.addEventListener('input',renderStudioProductList)});
