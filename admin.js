...
async function saveTubeTopInventory(){return withAdminLoading(async()=>{const sizes={};document.querySelectorAll('[data-tube-stock]').forEach(input=>{const size=input.dataset.tubeStock,available=document.querySelector(`[data-tube-available="${size}"]`);sizes[size]={stock:Math.max(0,Number(input.value||0)),available:!!available?.checked}});const product={...DATA.tubeTop,name:'Spandex Tube Top',price:Number(document.getElementById('tubeTopPriceAdmin')?.value||64),color:'Black',description:document.getElementById('tubeTopDescriptionAdmin')?.value||'Double lined and stretchy',sizes,updatedAt:new Date().toISOString()};await BFStore.setDoc('products/spandexTubeTop',product,false);DATA.tubeTop=product;renderTubeTopInventory();renderInventorySummary();BF.toast('Spandex Tube Top stock saved.');},'Saving tube top stock…')}
...
function renderCatalogProducts(){const el=document.getElementById('catalogAdminList');if(!el)return;el.innerHTML=(DATA.catalog||[]).filter(x=>['ribbed','tops','sets'].includes(x.category)&&x.id!=='spandex-tube-top').map(item=>`<article class="admin-catalog-card" data-catalog-id="${item.id}"><div class="admin-catalog-grid"><div class="admin-field"><label>Product name</label><input data-field="name" value="${escapeAdminValue(item.name)}"></div><div class="admin-field"><label>Category</label><select data-field="category"><option value="ribbed" ${item.category==='ribbed'?'selected':''}>Ribbed</option><option value="tops" ${item.category==='tops'?'selected':''}>Tops</option><option value="sets" ${item.category==='sets'?'selected':''}>Sets</option></select></div><div class="admin-field"><label>Price (GHS)</label><input data-field="price" type="number" min="0" value="${item.price??(item.category==='ribbed'?DATA.settings.ribbedPrice||'':'')}"><p class="field-help">Ribbed items can share the store Ribbed price, or you can set an item-specific price.</p></div><div class="admin-field"><label>Colour / pack colours</label><input data-field="color" value="${escapeAdminValue(item.color||'')}"></div><div class="admin-field" style="grid-column:1/-1"><label>Description</label><textarea data-field="description">${escapeAdminValue(item.description||'')}</textarea></div><div class="admin-field"><label>Available?</label><select data-field="available"><option value="true" ${item.available!==false?'selected':''}>Yes</option><option value="false" ${item.available===false?'selected':''}>No</option></select></div><div class="admin-field"><label>Image key (edit path in images.js)</label><input data-field="imageKey" value="${escapeAdminValue(item.imageKey||item.id)}"></div></div>${item.sizes?`<div class="admin-size-list"><strong>Sizes & stock</strong><div data-size-rows>${Object.entries(item.sizes).map(([size,d])=>`<div class="admin-size-row"><input data-size-name value="${escapeAdminValue(size)}"><input data-size-stock type="number" min="0" value="${Number(d.stock||0)}"><label class="availability-check"><input data-size-available type="checkbox" ${d.available===false?'':'checked'}><span>Sell</span></label></div>`).join('')}</div><button class="small-btn" type="button" onclick="addCatalogSizeRow('${item.id}')">+ Add size</button><div class="admin-field"><label>Size description</label><textarea data-field="sizeDescription">${escapeAdminValue(item.sizeDescription||'')}</textarea></div></div>`:`<div class="admin-field"><label>Stock</label><input data-field="stock" type="number" min="0" value="${Number(item.stock??0)}"></div>`}</article>`).join('')}
function addCatalogSizeRow(id){const card=document.querySelector(`[data-catalog-id="${id}"]`),rows=card?.querySelector('[data-size-rows]');if(!rows)return;rows.insertAdjacentHTML('beforeend','<div class="admin-size-row"><input data-size-name placeholder="New size"><input data-size-stock type="number" min="0" value="0"><label class="availability-check"><input data-size-available type="checkbox" checked><span>Sell</span></label></div>')}
window.addCatalogSizeRow=addCatalogSizeRow;
async function saveCatalogProducts(){return withAdminLoading(async()=>{const existing=Object.fromEntries((DATA.catalog||[]).map(x=>[x.id,x]));document.querySelectorAll('[data-catalog-id]').forEach(card=>{const id=card.dataset.catalogId,item={...(existing[id]||{}),id};card.querySelectorAll('[data-field]').forEach(input=>{const k=input.dataset.field;if(k==='price')item[k]=input.value===''?null:Number(input.value);else if(k==='stock')item[k]=Math.max(0,Number(input.value||0));else if(k==='available')item[k]=input.value==='true';else item[k]=input.value});const rows=[...card.querySelectorAll('.admin-size-row')];if(rows.length){item.sizes={};rows.forEach(r=>{const size=r.querySelector('[data-size-name]')?.value.trim();if(size)item.sizes[size]={stock:Math.max(0,Number(r.querySelector('[data-size-stock]')?.value||0)),available:!!r.querySelector('[data-size-available]')?.checked}})}existing[id]=item});const items=Object.values(existing);await BFStore.setDoc('products/catalog',{items,updatedAt:new Date().toISOString()},false);await BFStore.log('Catalog products updated');BF.toast('Ribbed, tops and sets saved.');await loadAll()},'Saving catalog products…')}
